...
CEgModuleVersion::CEgModuleVersion()
{
...
BOOL CEgModuleVersion::GetModuleVersionInfo(HINSTANCE hInstance)
{
...
BOOL CEgModuleVersion::GetModuleVersionInfo(LPCTSTR szModuleName)
{
...
BOOL CEgModuleVersion::GetFileVersionInfo(LPCTSTR szFileName)
{
...
//////////////////
// Get CString file info.
// Key name is something like "CompanyName".
// returns the value in szValue.
BOOL CEgModuleVersion::GetValue(LPCTSTR szKeyName, _bstr_t& sVal)
{
	BOOL	bRet = FALSE;
	...
	if (m_pVersionInfo)
	{
		// To get a CString value must pass query in the form
		//
...
typedef HRESULT (_stdcall* DLLGETVERSIONPROC)(DLLVERSIONINFO *);

/////////////////
// Get DLL Version by calling DLL's DllGetVersion proc
BOOL CEgModuleVersion::DllGetVersion(LPCTSTR szModuleName, DLLVERSIONINFO& dvi)
{
