...
BOOL CPictureEx::Draw()
{
	...
	if (IsAnimatedGIF())
	{
		TFrame & frm = m_arrFrames[m_nFrame];
		if ( (m_nFrame == 0) || (m_nFrame > 0  && frm.m_nDisposal != 2) )
		{
			bResult = RenderPicture (m_arrFrames[0].m_pPicture, CPoint (0, 0), m_PictureSize);
		}

		if (m_nFrame > 0)
			bResult = RenderPicture (frm.m_pPicture, frm.m_frameOffset, frm.m_frameSize);
	}
	else
		bResult = RenderPicture (m_pPicture, CPoint (0, 0), m_PictureSize);
...
BOOL  CPictureEx::RenderPicture (IPicture  * pPicture, CPoint pt, CSize size)
{
	if (! pPicture)
		return FALSE;
	...
	hr = pPicture->Render(m_hMemDC, pt.x, pt.y, size.cx, size.cy, 
...
BOOL CPictureEx::Load(LPCTSTR szResourceName, LPCTSTR szResourceType, HMODULE hModule)
{
	ASSERT(szResourceName);
	ASSERT(szResourceType);

	HRSRC hPicture = FindResource(hModule, szResourceName, szResourceType);
	HGLOBAL hResData;
	if (!hPicture || !(hResData = LoadResource(hModule, hPicture)))
	{
		TRACE(_T("Load (resource): Error loading resource %s\n"),szResourceName);
		return FALSE;
	};
	DWORD dwSize = SizeofResource(hModule, hPicture);
...
void CPictureEx::ThreadAnimation()
{
	while (true)
	{
		if (m_arrFrames[m_nFrame].m_pPicture)
		{
		///////////////////////////////////////////////////////
		...
		//   3 - restore to previous
			...
			if (m_arrFrames[m_nFrame].m_nDisposal == 3)
			{
				...
				hMemBM = CreateCompatibleBitmap(m_hMemDC,
							m_arrFrames[m_nFrame].m_frameSize.cx,
							m_arrFrames[m_nFrame].m_frameSize.cy);
				...
				if (hMemDC && hMemBM)
				{
					...
					BitBlt(hMemDC,0,0,
						...
						m_arrFrames[m_nFrame].m_frameSize.cy,
						m_hMemDC,
						m_arrFrames[m_nFrame].m_frameOffset.cx,
						m_arrFrames[m_nFrame].m_frameOffset.cy,
						SRCCOPY);
				};
			};
			///////////////////////
			...
			long hmWidth;
			long hmHeight;
			m_arrFrames[m_nFrame].m_pPicture->get_Width(&hmWidth);
			m_arrFrames[m_nFrame].m_pPicture->get_Height(&hmHeight);

			if (m_arrFrames[m_nFrame].m_pPicture->Render(m_hMemDC, 
				m_arrFrames[m_nFrame].m_frameOffset.cx, 
				m_arrFrames[m_nFrame].m_frameOffset.cy, 
				m_arrFrames[m_nFrame].m_frameSize.cx, 
				m_arrFrames[m_nFrame].m_frameSize.cy, 
...
*/
			...
			if (m_arrFrames[m_nFrame].m_nDelay < 5) 
				dwTimeOut = 10 * 10;
			else
				dwTimeOut = m_arrFrames[m_nFrame].m_nDelay * 10;


			// of course, we could use a casual Sleep()
			// but then Stop() would take quite some time
			DWORD dwResult = WaitForSingleObject( m_hExitEvent,  dwTimeOut );
...
/*				RECT rect = {0,0,m_PictureSize.cx,m_PictureSize.cy};
				...
				Draw (); // Display first frame;
*/
				break;
			};

			// disposal method #2
			if (m_arrFrames[m_nFrame].m_nDisposal == 2)
			{
				HBRUSH hBrush = CreateSolidBrush(m_clrBackground);
				if (hBrush)
				{
					RECT rect = {
						m_arrFrames[m_nFrame].m_frameOffset.cx,
						m_arrFrames[m_nFrame].m_frameOffset.cy,
						m_arrFrames[m_nFrame].m_frameOffset.cx + m_arrFrames[m_nFrame].m_frameSize.cx,
						m_arrFrames[m_nFrame].m_frameOffset.cy + m_arrFrames[m_nFrame].m_frameSize.cy };
					FillRect(m_hMemDC,&rect,hBrush);
					DeleteObject(hBrush);
				};
			} 
			else
				if (hMemDC && (m_arrFrames[m_nFrame].m_nDisposal == 3) )
				{
					// put it back
					BitBlt(m_hMemDC,
						m_arrFrames[m_nFrame].m_frameOffset.cx,
...
void CPictureEx::Stop(bool bReset)
{
...
BOOL CPictureEx::Start()
{
	if (! IsAnimatedGIF ()  ) 
	{
		return FALSE;
		TRACE(_T("Start: not  a GIF animation\n"));
	}
