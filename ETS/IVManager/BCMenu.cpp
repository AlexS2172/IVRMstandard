...
if (szWideString)
    {
	int lSize = sizeof(wchar_t)*(wcslen(szWideString)+1);	
    m_szMenuText = new wchar_t[lSize];
    if (m_szMenuText)
        wcscpy_s(m_szMenuText, lSize, szWideString);
    }
else
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
  ...
  do{
    ...
    nLen = 0;
	long lStringSize = wcslen((wchar_t *)pTp)+1;
    szCaption=new wchar_t[lStringSize];
    wcscpy_s(szCaption,lStringSize, (wchar_t *)pTp);
    pTp=&pTp[lStringSize*sizeof(wchar_t)];//modified SK
...
//------------------
// VKL
BOOL BCMenu::SetMenuText(UINT id, LPCTSTR szText, UINT nFlags)
{
	BOOL returnflag=FALSE;
...
#ifdef _UNICODE			
			m_MenuList[id]->SetWideString(szText);
#else
			m_MenuList[id]->SetAnsiString(szText);
#endif 
			returnflag=TRUE;
		}
	}
	else
	{
		int uiLoc;
		BCMenu* pMenu = FindMenuOption(id,uiLoc);
		if(NULL!=pMenu) returnflag = pMenu->SetMenuText(uiLoc,szText);
	}
	return(returnflag);
}
                             ...
                             BOOL bSelected, HBITMAP hbmCheck)
{
  ...
  if (! bSelected)
  {
	  COLORREF clrBack = dc->SetBkColor (::GetSysColor(COLOR_3DHILIGHT) ); 	
	  COLORREF clrText = dc->SetTextColor (::GetSysColor(COLOR_3DFACE) );
	  
	  CBrush * pBrush = dc->GetHalftoneBrush();
  ...
  {
	  CBrush brush;
	  brush.CreateSysColorBrush (COLOR_3DLIGHT);
