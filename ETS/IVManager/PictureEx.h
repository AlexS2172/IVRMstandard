...
public:
	BOOL Start(); // starts an animation thread if needed
	CPictureEx();
	virtual ~CPictureEx();
	void Stop(bool bReset = true);   // stops animation
	...
	BOOL Load(LPCTSTR szResourceName,LPCTSTR szResourceType, HMODULE hModule = NULL);

protected:
	// draws the picture 
	BOOL Draw();
	BOOL RenderPicture (IPicture  * pPicture, CPoint pt, CSize size);
