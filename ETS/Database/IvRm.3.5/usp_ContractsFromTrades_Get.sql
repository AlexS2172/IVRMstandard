...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_ContractsFromTrades_Get]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[usp_ContractsFromTrades_Get]
GO

create  procedure dbo.usp_ContractsFromTrades_Get
As
	...
	-------------------------------------------
	select
		distinct cv.ContractID as iContractID,
	...
	select
		...
		distinct fc.contractID 		as iContractID,
