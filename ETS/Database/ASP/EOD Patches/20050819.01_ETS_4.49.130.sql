...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '4.49.129' or (keyValue = '4.49.130')))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '4.49.130 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '4.49.130' as  'Version in script' from DataInfo where dataInfoID = 1
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '4.49.130')
	begin
		update DataInfo set keyValue = '4.49.130' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
IF @@ERROR <> 0 AND @@TRANCOUNT > 0  ROLLBACK TRANSACTION
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 4.49.130' '4.49.130 update message:' 
