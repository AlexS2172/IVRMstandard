...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.46.242' or (keyValue = '3.46.243') ))
BEGIN
	select 'You can''t run this script, because you have not run script for previous version!' '3.46.243 error message:' 
	Select cast(keyValue as varchar(25)) as 'DB version', '3.46.243' as  'Version in script' from DataInfo where dataInfoID = 1
...
/*  OPTION (MAXDOP 1) is added */
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[usp_AspEodUpdateMarketStructure_DefaultIVs_Execute]')
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.46.243')
	begin
		update DataInfo set keyValue = '3.46.243' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.46.243' '3.46.243 update message:' 
