...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.46.200'  or (keyValue = '3.46.201') ))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.46.201 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.46.201' as  'Version in script' from DataInfo where dataInfoID = 1
...
GO
ALTER  PROCEDURE dbo.usp_BoTraderUnderlyings_Get
	@iTraderID int,
...
AS
	...
	begin
		...
		where C.contractTypeID in (1,2,6) and TC.traderID=@iTraderID
		order by C.symbol
	end	
	return @@error
...
*/
	...
	Begin Transaction CommodityDel
		...
		update FutureUnderlying set actionID=3 where iFutureUnderlyingID = @iCID
		if @@error <> 0 begin set @error = -3 if @@trancount <> 1 rollback tran CommodityDel goto finish end
...
select contractID
from contract where  contractTypeid = 4
and contractID not in (select FutureID from future)
)
GO
...
GO
delete from ContractExchangeLink where ContractID in (
select contractID
from contract where  contractTypeid = 4
and contractID not in (select FutureID from future)
)
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.46.201')
	begin
		update DataInfo set keyValue = '3.46.201' where dataInfoID = 1
	end
GO
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.46.201' '3.46.201 update message:' 
