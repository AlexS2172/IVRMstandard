...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.46.174'  or (keyValue = '3.46.175') ))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.46.175 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.46.175' as  'Version in script' from DataInfo where dataInfoID = 1
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.46.175')
	begin
		update DataInfo set keyValue = '3.46.175' where dataInfoID = 1
	end
GO
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.46.175' '3.46.175 update message:' 
