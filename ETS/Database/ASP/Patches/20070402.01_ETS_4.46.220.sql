...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.46.219'  or (keyValue = '3.46.220') ))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.46.220 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.46.220' as  'Version in script' from DataInfo where dataInfoID = 1
...
As
   select Cn.symbol, Op.optionid As OptionID , T.TotalSum  As TotalSum ,  Op.expiryDate --ec.expirydate 
	...
	where datediff ( dd , /*ec.expirydate*/ Op.expiryDate , getdate() ) >= @vOptWithPsExp -- and Op.actionid < 3 
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.46.220')
	begin
		update DataInfo set keyValue = '3.46.220' where dataInfoID = 1
	end
GO
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.46.220' '3.46.220 update message:' 
