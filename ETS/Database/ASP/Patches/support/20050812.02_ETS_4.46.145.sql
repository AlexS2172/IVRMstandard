...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.46.144'  or (keyValue = '3.46.145') ))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.46.145 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.46.145' as  'Version in script' from DataInfo where dataInfoID = 1
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.46.145')
	begin
		update DataInfo set keyValue = '3.46.145' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.46.145' '3.46.145 update message:' 
