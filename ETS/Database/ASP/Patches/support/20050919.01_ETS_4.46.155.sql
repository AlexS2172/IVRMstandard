...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.46.154'  or (keyValue = '3.46.155') ))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.46.155 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.46.155' as  'Version in script' from DataInfo where dataInfoID = 1
...
AS
	...
	delete from Trade 
		where status in (0, 3) and isPosition = 0 and DateDiff(day, tradeDate, @dtTrade) >= 0
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.46.155')
	begin
		update DataInfo set keyValue = '3.46.155' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.46.155' '3.46.155 update message:' 
