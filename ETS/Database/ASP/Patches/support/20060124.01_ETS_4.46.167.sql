...
AS
	...
	if @iContractTypeID = 4 and @vcUnderlyingSymbol is null
        begin
		select @vcUnderlyingSymbol = C.Symbol from Contract C
			inner join FutureRoot F on F.FutureRootSymbol = @vcOptRootSymbol and F.underlyingID = C.ContractID 
		where C.contractTypeID < 3
		if @vcUnderlyingSymbol is NULL
		begin
			select @vcUnderlyingSymbol = C.Symbol from ETS_ASP_EOD.dbo.Contract C
				inner join ETS_ASP_EOD.dbo.FutureRoot F on F.FutureRootSymbol = @vcOptRootSymbol and F.underlyingID = C.ContractID 
			where C.contractTypeID < 3
			if @vcUnderlyingSymbol is NULL
			begin
				-- unknown underlying
	...
	if @iContractTypeID = 3
		...
		if @iContractTypeID = 4
			begin
				...
				Select @iFutureRootID = futureRootID from FutureRoot where FutureRootSymbol = @vcOptRootSymbol
		
				set @iExpiryID = (year(@dtExpiry) - 2000) * 12 + month(@dtExpiry)
	
				select @iFutureID = F.FutureID
					from   Future F, Expiry E
					where F.maturityID = E.expiryID and
						F.FutureRootID = @iFutureRootID and 
						E.expiryID = @iExpiryID and E.actionID < 3
				...
				--create new future
					begin
						...
						begin
							...
							insert into Contract (contractTypeID, symbol, contractName) values (4, @vcContractSymbol,'')
								if @@error <> 0 begin set @error = 2 if @@TranCount > 0 Rollback Transaction goto finish end
								set @iFutureID = @@identity
							
							if @iFutureRootID is null
								begin
									insert into FutureRoot(underlyingID,futureRootSymbol,futureLotSize) values(@iUnderlyingContractID,@vcOptRootSymbol, isnull(@iTradeLotSize, 100))
										set @iFutureRootID = @@identity
								end
							insert into Future (FutureID, FutureRootID, maturityID)
								 values (@iFutureID, @iFutureRootID, @iExpiryID)
								if @@error <> 0 begin set @error = 2 if @@TranCount > 0 Rollback Transaction goto finish end
							-- make record with null exchange for this Future
							insert into ContractPrice(contractID,exchangeID) values(@iFutureID,null)
								if @@error <> 0 begin set @error = 2 if @@TranCount > 0 Rollback Transaction goto finish end
						end
						else
						begin
	...
	values (@vcExecID, @iNewTradeID, 1, @iTradeContractID, @dtTradeDate, 
		...
		@iUnderlyingContractID, @iOptionRootID, @iFutureRootID, @ifutureID, @fStockRefPrice )
