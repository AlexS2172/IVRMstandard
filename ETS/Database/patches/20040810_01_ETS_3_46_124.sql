...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.46.123' or (keyValue = '3.46.124')))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.46.124 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.46.124' as  'Version in script' from DataInfo where dataInfoID = 1
...
IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'usp_Stock_Import' 
	   AND 	  type = 'P')
    DROP PROCEDURE usp_Stock_Import
...
CREATE proc dbo.usp_Stock_Import
	...
	@fPriceAsk float = -1, -- fPriceAsk < 0 означает, что остальные параметры игнорировать
	@fPriceBid float = 0,
	@fPriceOpen float = 0,
	@fPriceClose float = 0,
	@fPriceLow float = 0,
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.46.124')
	begin
		update DataInfo set keyValue = '3.46.124' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
IF @@ERROR <> 0 AND @@TRANCOUNT > 0  ROLLBACK TRANSACTION
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.46.124' '3.46.124 update message:' 
