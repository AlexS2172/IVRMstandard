/*
additional fix for MLine00001220
*/
...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.41.068' or (keyValue = '3.41.069')))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.41.069 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.41.069' as  'Version in script' from DataInfo where dataInfoID = 1
	...
	insert into #Options_for_delete
		select Contract.contractID 
			...
			where symbol = @vcSymbol and contractTypeID = 3 and 
				...
				and optionRootID!=@iOptionRootID	if exists(select * from #Options_for_delete)
		begin
			exec usp_Option_Del
		end
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.41.069')
	begin
		update DataInfo set keyValue = '3.41.069' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
IF @@ERROR <> 0 AND @@TRANCOUNT > 0  ROLLBACK TRANSACTION
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.41.069' '3.41.069 update message:' 
