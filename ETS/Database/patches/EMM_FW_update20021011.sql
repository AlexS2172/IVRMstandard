...
*/
	...
	--add new stock\index
		begin
			...
			insert into ContractPrice (contractID,exchangeID,lotSize) values(@iUnderlyingContractID,null,1)
				if @@error <> 0 begin set @error = 2 if @@TranCount = 1 Rollback Transaction goto finish end
			...
			--create new option
				begin
					...
					begin
						...
						insert into ContractPrice(contractID,exchangeID,lotSize) values(@iOptionID,null,isnull(@iTradeLotSize,100))
							if @@error <> 0 begin set @error = 2 if @@TranCount = 1 Rollback Transaction goto finish end
...
--change DB version
--===================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.37.032')
	begin
		update DataInfo set keyValue = '3.37.032' where dataInfoID = 1
		print 'DB VERSION WAS CHANGED TO 3.37.032'
	end
