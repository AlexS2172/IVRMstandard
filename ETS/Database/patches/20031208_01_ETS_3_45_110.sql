...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.45.109' or (keyValue = '3.45.110')))
	BEGIN
		select 'You can''t run this script, because you have not run script for previous version!' '3.45.110 error message:' 
		Select cast(keyValue as varchar(25)) as 'DB version', '3.45.110' as  'Version in script' from DataInfo where dataInfoID = 1
...
----------------------------------------------------------------------------------------------------
-- Clear bad ExportSymbol from FutureRoot table
----------------------------------------------------------------------------------------------------
update FutureRoot set exportSymbol = null
GO
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.45.110')
	begin
		update DataInfo set keyValue = '3.45.110' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
IF @@ERROR <> 0 AND @@TRANCOUNT > 0  ROLLBACK TRANSACTION
...
IF @@TRANCOUNT > 0 
BEGIN
	select 'DB VERSION WAS CHANGED TO 3.45.110' '3.45.110 update message:' 
