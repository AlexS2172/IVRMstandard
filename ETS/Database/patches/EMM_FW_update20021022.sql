/*
Additional fix for MLine00000831
*/
...
if not exists(select * from DataInfo where dataInfoID = 1 and (keyValue = '3.37.034' or (keyValue = '3.37.035')))
	BEGIN
		PRINT 'You can''t run this script, because you have not run script for previous version!'
		Select cast(keyValue as varchar(25)) as 'DB version', '3.37.035' as  'Version in script' from DataInfo where dataInfoID = 1
...
--=============================================================================================
if not exists(select * from DataInfo where DataInfoID = 1 and keyValue = '3.37.035')
	begin
		update DataInfo set keyValue = '3.37.035' where dataInfoID = 1
	end
GO
/* RESULTS ANALYTHING */
IF @@ERROR <> 0 AND @@TRANCOUNT > 0  ROLLBACK TRANSACTION
...
BEGIN
	...
	PRINT 'DB VERSION WAS CHANGED TO 3.37.035'
	COMMIT TRANSACTION
END
