...
Private Declare Function CalcCRC32 Lib "edaTools.dll" (lpData As Any, ByVal nDataLen As Long) As Long
...
Public Function GetLicenseByKey(ByVal sKey As String, ByRef lcLicense As LicenseType) As Boolean
...
Private Sub DecodeExpireDate(ByVal lCode As Long, ByRef lcLicense As LicenseType)
    On Error Resume Next
...
Private Function CheckLicData(ByRef lcLicense As LicenseType) As Boolean
    On Error Resume Next
    CheckLicData = False
...
Public Function CalcCRC(ByVal sData As String, ByRef frmOwner As Form) As Long
    On Error GoTo EH
    Dim nLen As Long
    CalcCRC = 0
    nLen = Len(sData)
    ...
    Exit Function
EH:
    gCmn.ErrorMsgBox frmOwner, "Fail to get license."
End Function

Public Function GetVolumeSerialNo(ByVal sPathName As String) As Long
    On Error Resume Next
    GetVolumeSerialNo = 0
...
' server side code
'Public Function GetKeyByLicense(lcLicense As LicenseType, sKey As String) As Boolean
'    On Error Resume Next
...
'
'
'Private Function EncodeExpireDate(lcLicense As LicenseType) As Long
'    On Error Resume Next
'    EncodeExpireDate = 0
