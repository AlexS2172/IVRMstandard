...
Begin VB.Form frmVolaMonitor 
   ...
   Begin VB.PictureBox picBottom 
      ...
      Begin VB.Image imgStop 
         Height          =   240
         Left            =   2070
         MouseIcon       =   "frmVolaMonitor.frx":000C
         MousePointer    =   99  'Custom
         Picture         =   "frmVolaMonitor.frx":015E
         ToolTipText     =   "Cancel"
         Top             =   45
         Visible         =   0   'False
         Width           =   240
      ...
      Begin VB.Image imgStopDis 
         ...
         Height          =   240
         Left            =   2070
         Picture         =   "frmVolaMonitor.frx":02A8
         Top             =   45
         Visible         =   0   'False
         Width           =   240
      End
   End
   Begin EtsMM.ctlVolaMonitor ctlView 
      Height          =   2535
      Left            =   0
      TabIndex        =   0
      Top             =   0
   ...
   Begin VB.Menu mnuView 
      ...
      End
   End
   Begin VB.Menu mnuVola 
      Caption         =   "Volat&ility"
      Begin VB.Menu mnuVolaShiftSel 
         Caption         =   "&Shift Volatility..."
         Shortcut        =   {F6}
      End
      Begin VB.Menu mnuVolaShiftAll 
   ...
   End
End
Attribute VB_Name = "frmVolaMonitor"
...
Public Sub ShowDataByFilter(ByRef aFilters As EtsGeneralLib.EtsFilterData)
    ...
    If Not g_PerformanceLog Is Nothing Then _
        g_PerformanceLog.LogMmInfo enLogUserAction, "frmVolaMonitor ShowData Call", Me.Caption
    
    ctlView.ShowDataByFilter aFilters
    
    If Not g_PerformanceLog Is Nothing Then _
        g_PerformanceLog.LogMmInfo enLogUserAction, "frmVolaMonitor ShowData Done", Me.Caption
...
Public Sub Init(sKey As String)
    ...
    m_sCaption = ""
    m_sShortCaption = ""
    g_nVolaUntitledIdx = g_nVolaUntitledIdx + 1
    m_sFileName = "Untitled" & g_nVolaUntitledIdx & ".vvw"
    m_bIsNew = True
    m_sKey = sKey
...
Private Sub ctlView_OnStateChange()
    ...
    mnuVolaShiftSel.Enabled = ctlView.VolaShiftEnabled
    mnuVolaShiftAll.Enabled = mnuVolaShiftSel.Enabled
End Sub
...
Private Sub Form_Load()
    ...
    g_Params.SetIcon WND_VOLAMONITOR, Me.hWnd, False
...
Private Sub Save(ByVal sFilePath As String, ByVal sFileName As String)
    ...
    aStorage.SetStringValue "Type", "Value", "VolaMonitor"
...
Private Sub mnuFileSave_Click()
    ...
    If m_bIsNew Then
        With dlgCommon
            ...
            .CancelError = True
            .DefaultExt = ".vvw"
            .DialogTitle = "Save Volatility Monitor"
            .InitDir = ""
            .FileName = m_sFilePath & m_sFileName
            .Filter = "ETS Volatility Monitor (*.vvw)|*.vvw|All Files|*.*"
            .FilterIndex = 1
            On Error Resume Next
            .ShowSave
...
EH:
    Screen.MousePointer = vbDefault
    gCmn.ErrorMsgBox Me, "Fail to save volatility monitor."
End Sub
...
Private Sub mnuFileSaveAs_Click()
    ...
    With dlgCommon
        ...
        .CancelError = True
        .DefaultExt = ".vvw"
        .DialogTitle = "Save Volatility Monitor As"
        .InitDir = ""
        .FileName = m_sFilePath & m_sFileName
        .Filter = "ETS Volatility Monitor (*.vvw)|*.vvw|All Files|*.*"
        .FilterIndex = 1
...
Private Sub mnuFileOpen_Click()
    ...
    With dlgCommon
        ...
        .CancelError = True
        .DefaultExt = ".vvw"
        .DialogTitle = "Open Volatility Monitor"
        .FileName = ""
        .InitDir = m_sFilePath
        .Filter = "ETS Volatility Monitor (*.vvw)|*.vvw|All Files|*.*"
        .FilterIndex = 1
        ...
        If Err.Number = 0 Then
            ...
            aStorage.ReadData
            If aStorage.GetStringValue("Type", "Value") = "VolaMonitor" Then
            ...
            Else
                Screen.MousePointer = vbDefault
                gCmn.MyMsgBox Me, "Invalid volatility monitor file."
            End If
        End If
    End With
    Exit Sub
EH:
    Screen.MousePointer = vbDefault
    gCmn.ErrorMsgBox Me, "Fail to open volatility monitor."
End Sub

Private Sub mnuGraphicalPnL_Click()
    On Error Resume Next
    If Not g_PerformanceLog Is Nothing Then _
    g_PerformanceLog.LogMmInfo enLogUserAction, "Menu ""VolaMonitor->File->PnL Projections"" Enter. ", Me.Caption
    
    If ctlView.Filter(VFC_GROUP) = TYPE_UNDERLYING Then
        frmMain.ShowGraphicalPnL TYPE_UNDERLYING, ctlView.Filter(VFC_VALUE)
    Else
        frmMain.ShowGraphicalPnL TYPE_UNDERLYING
    End If
    
    If Not g_PerformanceLog Is Nothing Then _
    g_PerformanceLog.LogMmInfo enLogUserAction, "Menu ""VolaMonitor->File->PnL Projections"" Exit. ", Me.Caption
End Sub
...
Private Sub mnuVolaShiftAll_Click()
    On Error Resume Next
    ctlView.ShowShiftVolatilityAll
End Sub

Private Sub mnuVolaShiftSel_Click()
    On Error Resume Next
    ctlView.ShowShiftVolatility
End Sub
