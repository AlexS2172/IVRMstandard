...
'Public Const DEFAULT_TRADER_HEDGE_STATUS& = enMmShGroupStatusAll
...
Public Const ROL_UND& = 1&
Public Const ROL_POS& = 2&
...
'        MakeAppComponentsDescription = MakeAppComponentsDescription & " " & APP_TITLE & vbTab & App.Major & "." & App.Minor & "." & App.Revision & vbCrLf
'    End If
'
...
Public Sub ShowFormInTaskBar(ByRef aFrm As Form, Optional ByVal bShow As Boolean = True)
    ...
    If nExStyle <> nNewExStyle Then
        ...
        SetWindowPos aFrm.hwnd, 0, 0, 0, 0, 0, SWP_NOSIZE Or SWP_NOMOVE Or SWP_NOZORDER Or SWP_NOACTIVATE
    End If
End Sub
...
Public Sub SortEntCollByName(ByRef aEntColl As clsEntityColl)
    On Error Resume Next
    Dim arrItems() As SortItem, aEnt As clsEntityAtom, i&, nCount&
