...
Begin VB.Form frmGridLayout 
   ...
   Begin VB.CommandButton btnCstVarEditor 
   ...
   Begin VB.Frame fraColumn 
      ...
      Begin VB.PictureBox picPreview 
         ...
         Top             =   240
         Width           =   3795
         Begin EtsMMCv.ctlLabelEx lblPreview 
            Height          =   480
            Left            =   0
            TabIndex        =   37
            Tag             =   "noauto"
      ...
      Begin VB.PictureBox picAltPreview1 
         ...
         Top             =   1200
         Width           =   1845
         Begin EtsMMCv.ctlLabelEx lblAltPreview1 
            Height          =   480
            Left            =   0
            TabIndex        =   35
            Tag             =   "noauto"
      ...
      Begin VB.PictureBox picAltPreview2 
         ...
         Top             =   1200
         Width           =   1845
         Begin EtsMMCv.ctlLabelEx lblAltPreview2 
            Height          =   480
            Left            =   0
            TabIndex        =   33
            Tag             =   "noauto"
      ...
      Begin VB.CheckBox chkDelta 
         Caption         =   "Show delta when simulating"
         Height          =   375
         Left            =   2040
         TabIndex        =   28
         Top             =   2520
   ...
   Begin VB.Frame fraGrid 
      ...
      Begin VB.PictureBox picGridSelPreview 
         ...
         Top             =   1200
         Width           =   3795
         Begin EtsMMCv.ctlLabelEx lblGridSelPreview 
            Height          =   480
            Left            =   0
            TabIndex        =   21
            Tag             =   "noauto"
      ...
      Begin VB.CheckBox chkGridLineHoriz 
         ...
         Width           =   1935
      End
      Begin EtsMMCv.ctlLabelEx lblGridFont 
         Height          =   300
         Left            =   2070
         TabIndex        =   26
         Tag             =   "noauto"
...
Public Event OnGridChanged(ByVal enGridType As GridTypeEnum, ByVal nVarID As Long)
...
Public Sub Execute(ByVal enGridType As GridTypeEnum, ByVal nFirstColIdx As Long, ByVal nLastColIdx As Long, _
    ...
    btnCstVarEditor.Visible = (m_enGridType = GT_RISKS_TOTALS Or m_enGridType = GT_RISKS_POSITIONS Or _
                               m_enGridType = GT_GREEKS_UND_ROWS)
...
Private Sub btnCstVarEditor_Click()
    On Error Resume Next
    'Dim nID As Long
    Dim nLevel As Long
    Dim frmrisks As frmRiskView
    Dim frmGreeks As frmGreeksView
    
    Select Case m_enGridType
        Case GT_RISKS_TOTALS
            Set frmrisks = m_frmOwner
            nLevel = frmrisks.ctlView.GrpLevelID
            
            m_frmVarEditor.EditVariable frmrisks.ctlView.ContextID, nLevel, BAD_LONG_VALUE, Nothing
            
        Case GT_RISKS_POSITIONS
            Set frmrisks = m_frmOwner
            nLevel = frmrisks.ctlView.UndLevelID
            
            m_frmVarEditor.EditVariable frmrisks.ctlView.ContextID, nLevel, BAD_LONG_VALUE, Nothing
            
        Case GT_GREEKS_UND_ROWS
            Set frmGreeks = m_frmOwner
            nLevel = frmGreeks.ctlView.PosLevelID
            
            m_frmVarEditor.EditVariable frmGreeks.ctlView.ContextID, nLevel, BAD_LONG_VALUE, Nothing
    End Select
        ...
        With chkDelta
            If .Enabled = True Then
                If .Value = Checked Then
                    aCol.ShowDeltaInSimulation = True
                    m_bDirty = True
                Else
                    aCol.ShowDeltaInSimulation = False
                    m_bDirty = True
                End If
            End If
        End With
...
Private Sub chkSimulate_Click()
    ...
    If m_nFirstColIdx <= m_nCurColIdx And m_nCurColIdx <= m_nLastColIdx Then
        ...
        With chkSimulate
            If .Enabled = True Then
                If .Value = Checked Then
                    aCol.UseInSimulation = True
                    chkDelta.Value = Unchecked
                    chkDelta.Enabled = True
                    m_bDirty = True
                Else
                    aCol.UseInSimulation = False
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If Not m_frmVarEditor Is Nothing Then
        Unload m_frmVarEditor
        Set m_frmVarEditor = Nothing
    End If
...
Private Sub ShowPreview()
    ...
    If m_nFirstColIdx <= m_nCurColIdx And m_nCurColIdx <= m_nLastColIdx Then
        ...
        'simulation stuff
        If aCol.SupportSimulation = True Then
            chkSimulate.Enabled = True
            chkSimulate.Value = IIf(aCol.UseInSimulation, 1, 0)
            If aCol.UseInSimulation = True Then
                chkDelta.Enabled = True
                chkDelta.Value = IIf(aCol.ShowDeltaInSimulation, 1, 0)
            Else
                chkDelta.Enabled = False
                chkDelta.Value = Unchecked
            End If
...
Private Sub m_frmVarEditor_OnVariablesChanged(ByVal nNewID As Long)
    ...
    RaiseEvent OnGridChanged(m_enGridType, nNewID)
    DoEvents
    
    m_nLastColIdx = GetColumnsCount(m_enGridType) - 1
    g_DefGrids(m_enGridType).CopyTo m_gdGrid
...
Private Sub DisableControls()
    ...
    If m_enGridType = GT_GREEKS_UND_ROWS Then
        btnGridFont.Enabled = False
        btnGridLine.Enabled = False
        chkGridLineVert.Enabled = False
        chkGridLineHoriz.Enabled = False
        btnGridSelBack.Enabled = False
        btnGridSelFore.Enabled = False
        lblGridFont.Enabled = False
