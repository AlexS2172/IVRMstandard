...
Public Sub Init(ByVal bApplyFilter)
    ...
    With flxFilter
        ...
        .ColDataType(ESC_FLT_ITM) = flexDTDouble
        .ColFormat(ESC_FLT_ITM) = "#,##0.00"
        .TextMatrix(0, ESC_FLT_ITM) = "ITM"
...
'        .SelectionMode = flexSelectionByRow
    ...
    With fgExerScr
        ...
        .ColDataType(ESC_ITM_AMOUNT) = flexDTDouble
        .ColFormat(ESC_ITM_AMOUNT) = "#,##0.00"
        .TextMatrix(0, ESC_ITM_AMOUNT) = "ITM Amount"
    ...
    If Row > 0 Then
        ...
        If dStrike >= 0 Then
            ...
            If bCall Then
                 ...
                 fgExerScr.TextMatrix(Row, ESC_ITM_AMOUNT) = dUndPrice - dStrike
            Else
                 If dStrike > dUndPrice Then bATM = True
                 fgExerScr.TextMatrix(Row, ESC_ITM_AMOUNT) = dStrike - dUndPrice
            End If
...
Private Sub flxFilter_StartEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    m_sLastValue = flxFilter.TextMatrix(Row, Col)
End Sub
            ...
            If dPrice >= 0 Then
                ...
                If IsItIndex(iRowIterator) Then
                    ...
                    If dSOQ <= 0 Then
                        ...
                        'm_SattusBar.Caption = "Start opening quotation prices are missing for some indexes. Please enter proper SOQ price or last index price will be used insted of SOQ"
...
Private Sub FilterLoad(ByVal bApplyFilter As Boolean)
    ...
    With flxFilter
        ...
        .ColComboList(ESC_FLT_ITM) = "|#" & CStr(BAD_DUBLE_VALUE) & ";<All>"
        .TextMatrix(1, ESC_FLT_ITM) = "<All>"
...
Public Sub FilterUpdate()
    ...
    Dim dFltATM As Double, dATM As Double
    iRowShow = 0
    'Exit Sub
    dtFiltDate = flxFilter.TextMatrix(1, ESC_FLT_DATE)
    
    Err.Clear
    dFltATM = CDbl(flxFilter.TextMatrix(1, ESC_FLT_ITM))
    If Err.Number <> 0 Then dFltATM = BAD_DOUBLE_VALUE
    ...
    For iRow = 1 To fgExerScr.Rows - 1
        If fgExerScr.TextMatrix(iRow, ESC_EXPIRY) <> "" Then
            ...
            Err.Clear
            dATM = CDbl(fgExerScr.TextMatrix(iRow, ESC_ITM_AMOUNT))
            If Err.Number <> 0 Then dATM = BAD_DOUBLE_VALUE
             
            If (Int(dtLoop) = Int(dtFiltDate) Or Int(dtFiltDate) = 0) And (dATM >= dFltATM) Then
