...
'
'Private m_ExpandedOrd As New clsFilterAtomColl
'
...
'
'                    For Each aExec In aOrd.Execs
'                        .AddItem ""
'                        nRow = .Rows - 1
'
'                        Set aRowData = New clsOvRowData
'                        Set aRowData.Ord = aOrd
'                        Set aRowData.Exec = aExec
'
'                        .RowData(nRow) = aRowData
'                        .TextMatrix(nRow, OLC_KEY) = CStr(aOrd.ID) & "_" & CStr(aExec.ID)
'
'                        .IsSubtotal(nRow) = False
'                        .RowOutlineLevel(nRow) = OOL_EXEC
'
'                        OrderExecUpdate nRow, aExec
'                    Next
'
...
'                    If aRowData.Exec Is Nothing Then
...
'End Sub
'
'Private Sub OrdersUpdateExecColors()
'    On Error Resume Next
'    Dim nCol&, nRow&, aExec As EtsMmGeneralLib.MmOrderExecAtom
'
'    With fgOrd
'        m_GridLock(GT_ORDERS_LIST).LockRedraw
'
'        For nRow = 1 To .Rows - 1
'            Set aExec = .RowData(nRow).Exec
'            If Not aExec Is Nothing Then
'                Dim i&, nIdx&
'                i = 0
'                nIdx = m_gdOrd.Idx(0)
...
'End Sub
'
'Private Sub OrderExecUpdate(ByVal nRow As Long, ByRef aRowExec As EtsMmGeneralLib.MmOrderExecAtom, _
'                                Optional ByVal bUpdateColors As Boolean = False)
'    On Error Resume Next
'    Dim nCol&, aExec As EtsMmGeneralLib.MmOrderExecAtom
'    Dim bColorAlreadyUpdated As Boolean
'
'    With fgOrd
'        m_GridLock(GT_ORDERS_LIST).LockRedraw
'
'        If Not aRowExec Is Nothing Then
'            Set aExec = aRowExec
'        Else
'            Set aExec = .RowData(nRow).Exec
'        End If
...
'
'                    Case OLC_EXEC_ID
'                        .TextMatrix(nRow, nCol) = aExec.ExecID
'
'                    Case OLC_EXEC_STATUS
'                        .TextMatrix(nRow, nCol) = aExec.OrderExecStatusString
'
'                    Case OLC_EXEC_PRICE
'                        .TextMatrix(nRow, nCol) = aExec.ExecPrice
'
'                    Case OLC_EXEC_QTY
'                        .TextMatrix(nRow, nCol) = aExec.ExecQty
'
'                    Case OLC_EXEC_DATE
'                        .TextMatrix(nRow, nCol) = aExec.ExecDate
'
'                End Select
'
...
'    Dim aExec As EtsMmGeneralLib.MmOrderExecAtom, aOrd As EtsMmGeneralLib.MmOrderAtom
'
'    With fgOrd
'        m_GridLock(GT_ORDERS_LIST).LockRedraw
'
...
'    On Error Resume Next
'    If m_bInProc Then Exit Sub
'    Dim nRow&, nOrdRow&, aRowData As clsOvRowData, aExec As EtsMmGeneralLib.MmOrderExecAtom, bHide As Boolean
'
'    With fgOrd
'        m_GridLock(GT_ORDERS_LIST).LockRedraw
'
...
'                .RowOutlineLevel(nRow) = OOL_EXEC
'
'                OrderExecUpdate nRow, aExec, True
'            Next
'
'            bHide = Not CheckOrderFilter(aOrd)
'            .RowHidden(nOrdRow) = bHide
