...
Private m_Filter(MFC_GROUP To MFC_COLUMN_COUNT) As Long
...
Private Sub ctlSource_OnScreenRefresh(Res() As EtsMmRisksLib.MmRvMatrixCalcResultType, Axis() As RmAxisTypeEnum, Filter() As Long)
    On Error Resume Next
...
Private Sub SetFilterArray(ByRef Filter() As Long)
    On Error Resume Next
    Dim i As Long
    
    If LBound(Filter) <> MFC_GROUP Or UBound(Filter) <> MFC_COLUMN_COUNT Then Exit Sub
