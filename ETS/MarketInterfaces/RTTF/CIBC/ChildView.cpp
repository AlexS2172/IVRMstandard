...
void CChildView::OnDestroy() 
{
	...
	XMLParams.SetXMLLong(_T("ETS\\CIBCTradeFeedAdapter\\Log"), COLUMN_TYPE, GetColumnWidth(COLUMN_TYPE_ID));
	XMLParams.SetXMLLong(_T("ETS\\CIBCTradeFeedAdapter\\Log"), COLUMN_DATE, GetColumnWidth(COLUMN_DATE_ID));
	XMLParams.SetXMLLong(_T("ETS\\CIBCTradeFeedAdapter\\Log"), COLUMN_TIME, GetColumnWidth(COLUMN_TIME_ID));
	XMLParams.SetXMLLong(_T("ETS\\CIBCTradeFeedAdapter\\Log"), COLUMN_DESC, GetColumnWidth(COLUMN_DESC_ID));
...
BOOL CChildView::InsertTraceMessage(CTracer::MessageTypeEnum Type, LPCTSTR pszMessage)
{
	ASSERT(pszMessage);
	...
	if (-1 != iItem) 
	{
		...
		SetItemText(iItem, 3, pszMessage);
	}
