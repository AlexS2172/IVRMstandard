...
void CFrmMain::OnActionStart() 
{
	...
	UINT nID = 0;
	m_hThread = (HANDLE)_beginthreadex(NULL, 0, OConnorProc, this, 0, &nID);
	if (!m_hThread)
	{
...
{
	if (COConnor::m_hStopApplication)
		::SetEvent(COConnor::m_hStopApplication);
}
...
//--------------------------------------------------------------------------------------------------------------//
UINT __stdcall CFrmMain::OConnorProc(LPVOID pData)
{
	CFrmMain* pMain = reinterpret_cast<CFrmMain*>(pData);
	COConnor* m_pOConnor = new COConnor(pMain->GetSessionID());

	DWORD dwResult = ERROR_SUCCESS;
	if (m_pOConnor)
	{
		dwResult = m_pOConnor->Start();
		if (dwResult)
	    {
