...
public:
	...
	{ 
		if(CTracer::GetWindowHandle())
		{
			...
			_sntprintf(lpszErrorMsg, 512, _T("Pequote Connector: Download %s %d%%"), (LPCSTR)_bstr_t(sRemoteFilename), nTotalBytesPercent);
...
CFileConnector::CFileConnector(std::string strSessionID) 
	:m_Session(_T("Pequote connector"))
...
//	}
    ...
    if ( !(dblTmp = double(_ttol(sTmp))/1000000) ) 
    {
...
BOOL CFileConnector::DownloadTradesFile()
{
	...
	if(CTracer::GetWindowHandle())
		::SetWindowText(CTracer::GetWindowHandle(), "Pequote Connector");
