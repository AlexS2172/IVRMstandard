...
struct __MmQvUndAtom
{
	...
	__MmQvUndAtom()
		...
		m_bIsHTB(VARIANT_FALSE), /*m_bUseCustDivs(VARIANT_FALSE), m_nDivFreqCust(0L), m_dtDivDateCust(0.),*/
		/*m_dDivAmtCust(0.),*/ m_nQty(BAD_LONG_VALUE), m_nQtyInShares(BAD_LONG_VALUE),
		...
		m_dSkew(0.), m_dKurt(0.), m_bUseMidRates(VARIANT_FALSE), m_nPosExchID(0L),
		m_nSleepStep(0L), m_dUndPosForRates(0.), m_nOptPosExchID(0L)//, m_dSU_Price(BAD_DOUBLE_VALUE)
	{
	}
};
					...
					ICalculationParametrs*		pParams)
	{
		...
		if(WAIT_TIMEOUT != WaitForSingleObject(m_hMutex, 5000))
		{
			try
			{
				...
				m_divs					=divs;
				m_undQuote				=undQuote;
				m_spCalculationParametrs=pParams;
				bRet = true;
			}
			catch(...){}
...
public:
	...
	STDMETHOD(get_AtmVola)(IMmQvExpAtom* pExp, DOUBLE dUndPriceTolerance, EtsPriceRoundingRuleEnum enPriceRoundingRule, VARIANT_BOOL ManualEdit, LONG SurfaceID, DOUBLE* pVola);
