...
Begin VB.UserControl FlatButton 
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   180
   ToolboxBitmap   =   "ctlFlatButton.ctx":0000
   Begin VB.Image imgDisabledPicture 
      Height          =   375
      Left            =   0
      Top             =   960
      Visible         =   0   'False
...
'WARNING! DO NOT REMOVE OR MODIFY THE FOLLOWING COMMENTED LINES!
'MappingInfo=imgDownPicture,imgDownPicture,-1,Picture
Public Property Get DownPicture() As Picture
    Set DownPicture = imgDownPicture.Picture
End Property

Public Property Set DownPicture(ByVal New_DownPicture As Picture)
    Set imgDownPicture.Picture = New_DownPicture
    UserControl_Paint
    PropertyChanged "DownPicture"
End Property

'WARNING! DO NOT REMOVE OR MODIFY THE FOLLOWING COMMENTED LINES!
'MappingInfo=imgDisabledPicture,imgDisabledPicture,-1,Picture
Public Property Get DisabledPicture() As Picture
    Set DisabledPicture = imgDisabledPicture.Picture
End Property

Public Property Set DisabledPicture(ByVal New_DisabledPicture As Picture)
    Set imgDisabledPicture.Picture = New_DisabledPicture
    UserControl_Paint
...
'WARNING! DO NOT REMOVE OR MODIFY THE FOLLOWING COMMENTED LINES!
'MemberInfo=8,0,0,0
Public Property Get Style() As ebfStyle
    Style = m_Style
End Property

Public Property Let Style(ByVal New_Style As ebfStyle)
    m_Style = New_Style
    UserControl_Paint
...
On Error Resume Next
    m_BackColor = m_def_ButtonBackColor
    m_ForeColor = m_def_ButtonForeColor
...
On Error Resume Next
    ...
    m_BackColor = PropBag.ReadProperty("BackColor", m_def_ButtonBackColor)
    m_ForeColor = PropBag.ReadProperty("ForeColor", m_def_ButtonForeColor)
    ...
    Set imgPicture.Picture = PropBag.ReadProperty("Picture", Nothing)
    Set imgDownPicture.Picture = PropBag.ReadProperty("DownPicture", Nothing)
    Set imgDisabledPicture.Picture = PropBag.ReadProperty("DisabledPicture", Nothing)
...
On Error Resume Next
    ...
    Call PropBag.WriteProperty("BackColor", m_BackColor, m_def_ButtonBackColor)
    Call PropBag.WriteProperty("ForeColor", m_ForeColor, m_def_ButtonForeColor)
    ...
    Call PropBag.WriteProperty("DownPicture", imgDownPicture.Picture, Nothing)
    Call PropBag.WriteProperty("DisabledPicture", imgDisabledPicture.Picture, Nothing)
...
Dim iClientWidth As Long
Dim imgPic As Image
Dim iCurX As Long, iCurY As Long
On Error Resume Next
    ...
    With UserControl
        ...
        With imgPicture
            imgDownPicture.Width = .Width
            imgDownPicture.Height = .Height
            imgDisabledPicture.Width = .Width
            imgDisabledPicture.Height = .Height
        End With

        ' setting image
        If m_Style = fbStandard Then
            Set imgPic = Nothing
        Else
            Set imgPic = imgPicture
            If m_Pressed Then
                If imgDownPicture.Picture <> 0 Then
                    Set imgPic = imgDownPicture
                End If
            End If
            If Not .Enabled Then
                If imgDisabledPicture.Picture <> 0 Then
                    Set imgPic = imgDisabledPicture
                End If
            End If
        End If
        ...
        iClientWidth = .ScaleWidth - 2 * m_CaptionShift


        ' cutting long caption
        s = cap1 & cap2 & cap3
        s2 = s
        Do While (.TextWidth(s2) > iClientWidth) And (Len(s) > 0)
            s = Left(s, Len(s) - 1)
            s2 = s & "... "
        Loop
        ...
        Case fbAlignLeft
            iCurX = m_CaptionShift
        Case fbAlignRight
            iCurX = .ScaleWidth - .TextWidth(s2) - m_CaptionShift
        Case Else
            iCurX = (.ScaleWidth - .TextWidth(s2)) / 2
        End Select

        ' y position
        iCurY = (.ScaleHeight - .TextHeight(m_Caption)) / 2
        If (Not imgPic Is Nothing) Then
            If (imgPic.Picture <> 0) Then
                If (Len(m_Caption) > 0) Then
                    PaintPicture imgPicture.Picture, (.ScaleWidth - imgPic.Width) \ 2, (.ScaleHeight - imgPic.Height - .TextHeight(m_Caption) - 2) \ 2
                    iCurY = (.ScaleHeight - .TextHeight(m_Caption) - 4)
                Else
                    PaintPicture imgPicture.Picture, (.ScaleWidth - imgPic.Width) \ 2, (.ScaleHeight - imgPic.Height) \ 2
                End If
            End If
        End If
        ...
        If (GetFocus = UserControl.hWnd) And m_CanDrawFocus Then
            ...
            .DrawWidth = 1
            Line (1, 1)-(.ScaleWidth - 2, .ScaleHeight - 2), vbBlack, B
        End If
