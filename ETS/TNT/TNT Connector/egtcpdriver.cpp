								 ...
								 const unsigned short usServerPort)
{
    ...
    {
		hThread =(HANDLE) _beginthreadex(NULL, 0, WorkerThread, this, 0, &dwThreadId);
	...
	hThread = (HANDLE)_beginthreadex(NULL, 0, WsaEventThread, this, 0, &dwThreadId);

	if (hThread == INVALID_HANDLE_VALUE) 
			return GetLastError();

	m_hWSAEventThread = hThread;
										  ...
										  SOCKADDR_IN & addr_srv)
{
    ...
    addr_srv.sin_port = htons((unsigned short)usPort);
...
DWORD WINAPI CEgTcpDriver::ResolveName(long lAddress, char* const pName, int iLen)
{
	...
	size_t iHostLen = strlen(pAddr->h_name);

	if (iHostLen >= (size_t)iLen)
...
DWORD WINAPI CEgTcpDriver::Connect(const char * const pServer, const DWORD usPort)
{
	if (!m_bInited)
		return 0;//0 is needed for Transport
