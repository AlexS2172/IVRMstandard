...
Begin VB.Form frmEntExchangeProp 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "XXX Properties"
   ClientHeight    =   3795
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5205
   Icon            =   "frmEntExchangeProp.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   3795
   ...
   Begin VB.Image imgSave 
      Height          =   240
      Left            =   120
      Picture         =   "frmEntExchangeProp.frx":000C
      Top             =   3360
      Visible         =   0   'False
      Width           =   240
   End
End
Attribute VB_Name = "frmEntExchangeProp"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
' Edit holidays for an exchange
'
Option Explicit
...
Dim Selected As Boolean
    ...
    ' Load items list
    ' loading full holidays list
    Set rsFull = gDBW.usp_Holiday_Get(Null, Null)
    If Not rsFull Is Nothing Then
        With rsFull
            If .RecordCount > 0 Then
                ...
                Do While Not .EOF
                    ID = !iHolidayID
                    Title = UCase(Format(!dtHolidayDate, "dd/mmm")) & _
                        " - " & !vcHolidayName
                    With rsList
                        .AddNew
                        !ID = ID
    ...
    ' loading holidays list for a current exchange
    Set rsSelected = gDBW.usp_Holiday_Get(m_ItemID, Null)
...
Dim ID As Long
    ...
    With rsList
        ...
        If .RecordCount > 0 Then
            ...
            Do While Not .EOF
                ID = !ID
                ' deleting all holidays for a current exchange
                gDBW.usp_ExchangeHoliday_Del m_ItemID, ID
                .MoveNext
            Loop
        End If
    End With
    ...
    With rsList
        ...
        If .RecordCount > 0 Then
            ...
            Do While Not .EOF
                ID = !ID
                ' inserting selected holidays for a current exchange
                gDBW.usp_ExchangeHoliday_Save m_ItemID, ID
                .MoveNext
            Loop
        End If
    End With
