...
m_bTerminate(true)
{
	...
	m_hOptionsDone.Attach(CreateEvent(NULL,FALSE,FALSE,NULL));
}
...
{
	EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("GetOptions Enter %s"), _T(bsUnderlying));
	...
	EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("GetOptions Exit %s"), _T(bsUnderlying));
}

long COptions::_GetOptions()
{
	EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("_GetOptions Enter %s"), _T(m_dbaKey.dbaKey.symbol) );
	long nStatus = 0;
	try
	{
		m_bIsGotOptions = false;
		...
		ResetEvent(m_hOptionsDone);
		nStatus = dba(&m_blk, nFunc, &m_dbaKey.dbaKey, &nKeyLen, &m_optionsFilter, &nRecLen, 0);
		
		if (nStatus == DBA_ERR_NO_ERROR)
		{
			//EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("_GetOptions NoError") );
			EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("_GetOptions NoError, Waiting at _GetOptions Start"));	
			HANDLE hEvents[] ={
				m_hOptionsDone,
				m_optionsFilter.hThread
			};
			::WaitForMultipleObjects(2,hEvents,FALSE, INFINITE);
			EgLib::CEgLibTraceManager::Trace(LogCustom, __FUNCTION__ , _T("Waiting at _GetOptions End"));	
			if(m_bTerminate)
			{
				nStatus = DBA_TERMINATED;
				EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("_GetOptions nStatus = DBA_TERMINATED") );
			}
	...
	EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("_GetOptions Exit %s"), _T(m_dbaKey.dbaKey.symbol));
	return nStatus;
}
...
long COptions::_OptionsProc(ULONG ulFunction, DBA_KEY *pDbaKey, DBA_OPTIONS_FILTER_RECORD *pDbaRec, DWORD dwStatus) 
{
	...
	if (dwStatus == DBA_ERR_NO_ERROR)
	{
		if(!pDbaKey->exchangeCode[0] || pDbaKey->exchangeCode[0] == ' ')
		{
			...
			CreateSz(szName, (const char*)pDbaRec->priceRec.dbaRec.item.symbol, MAX_TICKER);
			//EgLib::CEgLibTraceManager::Trace(LogInfo, __FUNCTION__, szName);
...
DATE COptions::_CreateExpiryDate(INT nDaysTillExpiration)
{
//	SYSTEMTIME	stCurDate = { 0 };
//	DATE		dRet = 0.;
	EgLib::vt_date dtNow = EgLib::vt_date::GetCurrentDate(true);
	dtNow += EgLib::vt_date_span((long)nDaysTillExpiration,0L,0L,0L);
	return dtNow;
//	if (VariantTimeToSystemTime((LONG)EgLib::vt_date::GetCurrentDate(), &stCurDate))
//	{
//		INT nRelativeYear = (stCurDate.wMonth + nDaysTillExpiration%12) / 12;
//		
...
void COptions::CancelRequest()
{
	...
	{
		m_bTerminate = true;
		EgLib::CEgLibTraceManager::Trace(LogDebug, __FUNCTION__ , _T("CancelRequest Waiting Started"));	
		::WaitForSingleObject(m_optionsFilter.hThread, INFINITE);
		EgLib::CEgLibTraceManager::Trace(LogDebug, __FUNCTION__ , _T("CancelRequest Waiting Ended"));	
	}
}

void COptions::CopyOptionFilterToInfo(const DBA_OPTIONS_FILTER_RECORD& rec, _OptionInfo& Info)
{
	Info->ExpirationDate = _CreateExpiryDate(rec.daystillexp);			
...
{
	Result->ExpirationDate = _CreateExpiryDate(rec.daystillexp);			
...
void COptions::CopyOptionFilterToResult(const DBA_OPTIONS_FILTER_RECORD& rec, _OptionResultsEx& Result)
{
	...
	Result->Ask = Price2Dbl(rec.priceRec.dbaRec.priceType, rec.priceRec.dbaRec.ask.price);
	Result->Bid = Price2Dbl(rec.priceRec.dbaRec.priceType, rec.priceRec.dbaRec.bid.price);
	Result->Last = Price2Dbl(rec.priceRec.dbaRec.priceType, rec.priceRec.dbaRec.lastSale.price);
	Result->OpenInterest = rec.priceRec.dbaRec.openInterest;
	Result->Volume = rec.priceRec.dbaRec.volume;
	Result->YVolume = rec.priceRec.yestVolume;
