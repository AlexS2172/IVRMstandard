...
void CHyperFeedStructureProvider::OnRequest(CRequestBasePtr pRequest)
{
	...
	case _enRequestFuture:
		{
			EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Requesting Future Start"));	
			CFutureRequest* pFutureRequest = dynamic_cast<CFutureRequest*>(pRequest.get());
			if(pFutureRequest)
			{
				EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("GetFutureResultsbySymbol Call"));				
				GetFutureResultsBySymbol(pFutureRequest->m_recParams, false);
				EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("GetFutureResultsbySymbol Done"));				
			}
			EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Requesting Future Done"));	

		}
		break;
	case _enRequestFutures:
		{
			EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Requesting Futures"));				
			...
			EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Requesting Futures Done"));				
		}
		break;
	case _enRequestFuturesOption:
		{
			...
			if(pFuturesOptionRequest)
			{
				...
				if(nResult == DBA_ERR_NO_ERROR) // Sharky: try to resolve FT_CQ00001489
				{
					...
					EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("GetPrice for: Symbol - %s"), (LPCTSTR)bsSymbol );
					...
					if(nResult == DBA_ERR_NO_ERROR) // Sharky: try to resolve FT_CQ00001489
					{
						EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Got Price for: %s"), (LPCTSTR)bsSymbol);
						EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Requesting future options for: %s"), (LPCTSTR)bsSymbol);
						GetFutureOptionsResults(m_clCurrentFuture, false);
						EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Got future options for: %s"), (LPCTSTR)bsSymbol);
						
					}
					else
					{
						EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("GetPrice FAILED for: %s"), (LPCTSTR)bsSymbol);
		...
		EgLib::CEgLibTraceManager::Trace(LogSubs, __FUNCTION__ , _T("Request Futures Options Done"));
		break;
	}
}
