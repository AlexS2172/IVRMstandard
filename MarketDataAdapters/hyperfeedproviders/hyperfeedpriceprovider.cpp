...
void CHyperFeedPriceProvider::PublicLastQuote(const _QuoteUpdateParams* pParams, const CDBARecordPtr& pData)
{
	_QuoteUpdateResult results;
	CopyExtendedToResult(pData.get(), results);
	results->Exchange = (*pParams)->Exchange;
	CResponseBasePtr pResponse = CResponseBasePtr((CResponseBase*)new CQuoteResultResponse(_enResponseLast, pParams, results));
	PublicResponse(pResponse);
}

void CHyperFeedPriceProvider::PublicQuoteUpdate(const _QuoteUpdateParams* pParams, const CSubscriptionInfoPtr& pRec)
{
	_QuoteUpdateResult results;
	CopyExtendedToResult(pRec, results);
	results->Exchange = (*pParams)->Exchange;
	CResponseBasePtr pResponse =CResponseBasePtr((CResponseBase*) new CQuoteResultResponse(_enResponseRealtime, pParams, results));
	PublicResponse(pResponse);
}