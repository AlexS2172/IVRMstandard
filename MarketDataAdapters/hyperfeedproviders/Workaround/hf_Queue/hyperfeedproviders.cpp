...
HRESULT CExeModule::LoadProviderSettings()
{
	...
	if (dwRes == ERROR_SUCCESS)
	{
		...
		if(ERROR_SUCCESS != key.QueryValue(m_sMessageQueueName, _T("MessageQueue")))
		{
			EgStd::CEgTracingClass::TraceStatic(enlogFaults, __FUNCTION__ , _T("The 'MessageQueue' value is not set"));	
			hr = E_FAIL;
		}
	}
	return hr;
