...
{
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("GetFutureOptionsResults Enter"));	
	if(IsStreamRunning())
	{
		EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("GetFutureOptionsResults Exit - StreamRunning"));	
		return;
	}
	...
	for(futureoptionrootmap::iterator iterRoot = clFuture.m_mapRoots.begin(); iterRoot != clFuture.m_mapRoots.end(); iterRoot++)
	{
		...
		EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_GetFutureOptionsResultsByRoot: %s"), m_strCurrentRoot.c_str());	
		...
		{
			EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_GetFutureOptionsResultsByRoot Not Found: %s"), m_strCurrentRoot.c_str());	
	...
	{
		EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Public result"));	
		PublicLastFutureOption();
	}
	else if(!m_bIsGotOptions && !bIsGotCriticalError) 
	{
		EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Not found"));	
...
{
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_GetFutureOptionsResultsByRoot Enter: Symbol - %s"), szBaseSymbol);	
	if(IsStreamRunning())
	...
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_GetFutureOptionsResultsByRoot Exit: Symbol - %s, Code - %d"), szBaseSymbol, nResult);	
	return nResult;
}

long CFuturesOptions::StreamData(long nStatus, long nFunc)
{
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("StreamData enter"));	
	BOOL bRet = FALSE;
	...
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("StreamData Exit with code %d"), bRet);	
	return bRet;
}

void CFuturesOptions::_SetDbKey(const char* szBaseSymbol)
{	
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_SetDbKey Enter: Symbol - %s"), szBaseSymbol);	
	...
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_SetDbKey Exit: Symbol - %s"), szBaseSymbol);	
}

bool CFuturesOptions::_CheckIsFutureOptionSymbolValid(const char* szSymbol, bool& IsCall, double& dStrike)
{
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_CheckIsFutureOptionSymbolValid Enter: Symbol - %s, IsCall - %d, Strike - %d"), szSymbol , IsCall, dStrike);	
	...
	for(int nIdx = strlen(szSymbol) - 1; nIdx >= 0; nIdx--)
	{
		if(!isdigit(szSymbol[nIdx]))
		{
			if(!bIsGetStrike)
			{
				...
				EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_CheckIsFutureOptionSymbolValid Exit, is valid: Symbol - %s, IsCall - %d, Strike - %d"), szSymbol , IsCall, dStrike);	
				bIsGetStrike = true;
			}
			...
			{
				IsCall = false;
				EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_CheckIsFutureOptionSymbolValid Exit, is invalid: Symbol - %s, IsCall - %d, Strike - %d"), szSymbol , IsCall, dStrike);	
				return true;
			}
...
{
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_CheckIsFutureOptionSymbolOur Enter: Symbol - %s, Root - %s"), szSymbol , m_strCurrentRoot.c_str());	
	bool bRet = true;
	if(_strnicmp(szSymbol,(const char*)m_strCurrentRoot.c_str(), m_strCurrentRoot.length()))
	{
		bRet = false;
	}
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_CheckIsFutureOptionSymbolOur Exit %d"), bRet);	
	return bRet;;
}
