...
{
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("OnRequest Enter"));	
	switch(pRequest->m_enType)
	{
	case _enRequestStock:
		{
			CStockRequest* pStockRequest = dynamic_cast<CStockRequest*>(pRequest.get());
			EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Requesting Stock Start"));	
			if(pStockRequest)
			{
				EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Get stock info call"));	
				GetStockInfo(pStockRequest->m_recParams);
			}
			EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Requesting Stock Done: %s"), _T(pStockRequest->m_recParams->Stock));	
		}
		break;
	case _enRequestOption:
		{
			EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Requesting Option Start"));	
...
//				if(!IsCanceled())
//				{
//					EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Requesting Option Fundamental Call: %s"), _T(pOptionRequest->m_recParams->UnderlyingSymbol));	
//					LoadOptionFundamentalInfo(pOptionRequest->m_recParams->UnderlyingSymbol);
//				}
					
				if(!IsCanceled())
				{
					EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("GetOptions Call: %s"), _T(pOptionRequest->m_recParams->UnderlyingSymbol));	
					GetOptions(pOptionRequest->m_recParams->UnderlyingSymbol);
				}
				
			}
			EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("Requesting Option Done"));	
		}
		break;
	}
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("OnRequest Exit"));	
}
