...
{
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("GetOptions Enter %s"), _T(bsUnderlying));
	...
	EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("GetOptions Exit %s"), _T(bsUnderlying));
}
...
long COptions::_GetOptions()
{
	...
	try
	{
		...
		if (nStatus == DBA_ERR_NO_ERROR)
		{
			...
			EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("_GetOptions NoError, Waiting at _GetOptions Start"));	
			HANDLE hEvents[] ={
				m_hOptionsDone,
				m_optionsFilter.hThread
			};
			::WaitForMultipleObjects(2,hEvents,FALSE, INFINITE);
			EgStd::CEgTracingClass::TraceStatic(enlogCustom, __FUNCTION__ , _T("Waiting at _GetOptions End"));	
			if(m_bTerminate)
			{
...
void COptions::CancelRequest()
{
	...
	{
		m_bTerminate = true;
		EgStd::CEgTracingClass::TraceStatic(enlogSubs, __FUNCTION__ , _T("CancelRequest Waiting Started"));	
		::WaitForSingleObject(m_optionsFilter.hThread, INFINITE);
		EgStd::CEgTracingClass::TraceStatic(enlogCustomExt, __FUNCTION__ , _T("CancelRequest Waiting Ended"));	
	}
}
