...
void CHyperFeedMarketDataForIv::PublicLastQuote(const _QuoteUpdateParams& Params, const DBA_RECORD_3& rec)
{
	if (enSTK == Params->Type)
	{
		_StockForIvResult results;
		results.Init();
		results->Symbol = ::SysAllocString(Params->Symbol);
		CopyExtendedToIvResult(rec, results);
	...
	else if (enOPT == Params->Type)
	{
		_OptionForIvResult results;
		results.Init();
		results->Ticker = ::SysAllocString(Params->Symbol);
		results->UnderlyingSymbol = _bstr_t(m_clCurrentUnderlying.m_sSymbol.c_str()).copy();

		if (m_RequestDataTypes & enIvReqEODO)
		{
			_CopyOptionExtendedToIvResult(rec, results);
		}
		if (m_RequestDataTypes & enIvReqSODO)
		{
			CopyFundamentalInfoToIvResult(results);
