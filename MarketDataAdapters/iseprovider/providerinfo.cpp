...
STDMETHODIMP CProviderInfo::get_Provider(ProviderRole type, BSTR *pProgID)
{
	...
	case enStructureProvider:
		clType = CLSID_ISEStdStructureProvider;
		break;
	case enStructureInfo:
		clType = CLSID_ISEStdStructureInfo;
		break;
	case enPriceProvider: 
		clType = CLSID_ISEStdPriceProvider;
		break;
	case enPriceInfo:
		clType = CLSID_ISEStdPriceInfo;
		break;
	}
    ...
    *pProgID = SysAllocString(SUCCEEDED(hr) ? (BSTR)bsData : L"");
