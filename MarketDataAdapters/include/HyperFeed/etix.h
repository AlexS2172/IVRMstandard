...
#define IT_EQUITY_FUTURE        9
#define IT_EQUITY_FUTURE_OPTION 10
#define IT_MAXIMUM				IT_EQUITY_FUTURE_OPTION
...
#define	FT_CTRL_INCREMENT_STATS			55		//	increment/decrement statistics value
#define	FT_CTRL_RESET_SOURCE			56		//	Perform End of Day Reset on DataSource
...
#define QF_FILL_ZERO_BBO_BID		0x0010
#define QF_FILL_ZERO_BBO_BID_SIZE	0x0020
#define QF_FILL_ZERO_BBO_ASK		0x0040
#define QF_FILL_ZERO_BBO_ASK_SIZE	0x0080
...
#define EXA_RESET_RECORD	0x00400000L 
...
typedef struct _ETI_SUMMARY_BITS {
	...
	BIT_FIELD	resetRecord 	: 1;		// Instructs the engine to perform the roll operation for this record	
	BIT_FIELD	lastActivityDate: 1;	    // long date/time used for processing reset record messages.
	BIT_FIELD	ignoreZeroFields: 1;		// Instructs the engine to ignore 0 fields on updates.
	BIT_FIELD	reserved2	 	: 1;		
	BIT_FIELD	session 		: 1;		
...
#define HTI_ORDER_SOURCE_ID_EURONEXT "EURONX"
#define HTI_ORDER_SOURCE_ID_MILAN "MILAN"
#define HTI_ORDER_SOURCE_ID_SHANGHAI "SHANGI"
#define HTI_ORDER_SOURCE_ID_BRUT "BRUT"
...
typedef struct _ORDER_BITS {
	...
	BIT_FIELD		topOfBook       : 1;	// Used to indicate that this message is the top of the book.  Not available on all datasources
	BIT_FIELD		unused          : 8;
#elif defined( PQTENDIAN_BIG )
	BIT_FIELD		unused          : 8;
	BIT_FIELD		topOfBook       : 1;	// Used to indicate that this message is the top of the book.  Not available on all datasources
...
/* packet Type MT_PAGE */
#define	PAGE_UPDATE					0x0001
#define	PAGE_INSERT					0x0002
#define	PAGE_STALE					0x0004
#define	PAGE_SOFT_DELETE			0x0008
#define	PAGE_HARD_DELETE			0x0010
#define PAGE_ASCII					0x0020
#define PAGE_AT_END_OF_SEQ			0x0040
#define PAGE_HAS_PREV_KEY			0x0080
#define PAGE_HAS_NEXT_KEY			0x0100
#define PAGE_HAS_ROW_LENGTH			0x0200
#define PAGE_HAS_COLUMN_LENGTH		0x0400
#define PAGE_HAS_ROW				0x0800
#define PAGE_HAS_COL				0x1000

typedef struct _ETI_PAGE_BITS {
#if defined( PQTENDIAN_LITTLE )
	BIT_FIELD	update       		: 1;
	BIT_FIELD	insert       		: 1;
	BIT_FIELD	stale				: 1;
	...
	BIT_FIELD	hardDelete			: 1;
	BIT_FIELD	ascii				: 1;
	BIT_FIELD	atendofseq			: 1;
	BIT_FIELD	hasprevkey			: 1;
	BIT_FIELD	hasnextkey			: 1;
	BIT_FIELD	hasrowlength		: 1;
	BIT_FIELD	hascolumnlength		: 1;
	BIT_FIELD	hasrow				: 1;
	BIT_FIELD	hascol				: 1;
	BIT_FIELD	undefined			: 3;
#elif defined( PQTENDIAN_BIG )
	BIT_FIELD	undefined			: 3;
	BIT_FIELD	hascol				: 1;
	BIT_FIELD	hasrow				: 1;
...
typedef union _ETI_PAGE_FLAGS {
	ETI_PAGE_BITS	bits;
	USHORT			mask;
} ETI_PAGE_FLAGS;	// 2 bytes


typedef struct ETI_PAGE {
	ETI_PACKET_HEADER	header;			//26 Bytes
	DBA_PAGE_KEY		key;
	ETI_PAGE_FLAGS		flags;			// 2 Bytes
...
/* packet Type MT_SELF_DESCRIBING */
#define	SELF_DESCRIBING_UPDATE				0x0001
#define	SELF_DESCRIBING_INSERT				0x0002
#define	SELF_DESCRIBING_STALE				0x0004
#define	SELF_DESCRIBING_HF_DATA_DICTIONARY	0x0008
#define SELF_DESCRIBING_REMOVE				0x0010
#define SELF_DESCRIBING_PASSTHRU			0x0020


#define SELF_DESCRIBING_MAX_LENGTH  4096

#define SELF_DESCRIBING_FIELD_SEPARATOR	  0x1F    // The format of the data field will be NAME<FS>DATA<RS>
#define SELF_DESCRIBING_RECORD_SEPARATOR  0x1E	// where FS is the field separator and RS is the record separator.

typedef struct _ETI_SELF_DESCRIBING_BITS {
#if defined( PQTENDIAN_LITTLE )
	BIT_FIELD	update       		: 1;	// Update Data
	BIT_FIELD	insert       		: 1;	// Insert Data
	BIT_FIELD	stale				: 1;	// Mark These Fields as Stale
	BIT_FIELD	hfDataDictionary	: 1;	// Use HyperFeed's Data Dictionary i.e. Processor will update Hyperfeed's Internal Databases
	BIT_FIELD	remove				: 1;    // Remove Data
	BIT_FIELD   passthru			: 1;    // Pass This Data Through To the Client
	BIT_FIELD	undefined			: 10;
#elif defined( PQTENDIAN_BIG )
	BIT_FIELD	undefined			: 10;
	BIT_FIELD	passthru			: 1;
	BIT_FIELD	remove				: 1;
	BIT_FIELD	hfDataDictionary	: 1;
	BIT_FIELD	stale				: 1;
	BIT_FIELD	insert				: 1;
	BIT_FIELD	update				: 1;
#endif
} ETI_SELF_DESCRIBING_BITS;

typedef union _ETI_SELF_DESCRIBING_FLAGS {
	ETI_SELF_DESCRIBING_BITS	bits;
	USHORT						mask;
} ETI_SELF_DESCRIBING_FLAGS;	// 2 bytes


#define SELF_DESCRIBING_KEYTYPE_UNKNOWN		0	
#define SELF_DESCRIBING_KEYTYPE_INSTRUMENT  1		//Used for DBA_KEY
#define SELF_DESCRIBING_KEYTYPE_PAGE		2		//Used for DBA_PAGE
#define SELF_DESCRIBING_KEYTYPE_TEXT		3		//Used for keys that need no translation
#define SELF_DESCRIBING_KEYTYPE_NEWS		4		//Used for News Key
#define SELF_DESCRIBING_KEYTYPE_DD			5		//For Vendor Data Dictionary Elements
#define SELF_DESCRIBING_KEYTYPE_UNDEFINED   -1


typedef struct ETI_SELF_DESCRIBING_KEY {
	BYTE		key[64];
	LONG		keyType;						// For Hyperfeed Data Dictionary Items we need a way to identify the key.
} ETI_SELF_DESCRIBING_KEY;	

typedef struct SELF_DESCRIBING_DATA {
	ULONG							dataLength;
	UCHAR							data[SELF_DESCRIBING_MAX_LENGTH];
} ETI_SELF_DESCRIBING_DATA;	

typedef struct ETI_SELF_DESCRIBING {
	ETI_PACKET_HEADER				header;			//26 Bytes
	ETI_SELF_DESCRIBING_FLAGS		flags;			// 2 Bytes
	ETI_SELF_DESCRIBING_KEY			key;			// 64 Bytes
	ETI_SELF_DESCRIBING_DATA		data;
} ETI_SELF_DESCRIBING;							
...
typedef union _ETI_DATA {
	...
	ETI_PAGE				page;
	ETI_SELF_DESCRIBING		selfDescribing;
} ETI_DATA;
