...
#if (!defined ACTIV_FEED_API_COMMON_INTRADAY_BAR_H)
#define ACTIV_FEED_API_COMMON_INTRADAY_BAR_H
...
{
public:	
	#define ACTIV_INTRADAY_BAR_FLAG_LIST(d)			\
		d(0x01,	NONE_REGULAR_HOUR_TRADE_BIT)		\
		d(0x02, VWAP_ONLY_BIT)

	ACTIV_INTRADAY_BAR_FLAG_LIST(ACTIV_DECLARE_BYTES)

	/**
	 *	@brief	Constructor.
	 */
	ACTIV_FEED_API_COMMON_API IntradayBar();

	/**
	 *	@brief	Destructor.
	 */
	ACTIV_FEED_API_COMMON_API virtual ~IntradayBar();
	 ...
	 */
	ACTIV_FEED_API_COMMON_API bool Compare(const IntradayBar &rhs) const;
	...
	/**
	 *	@brief	Check if the bar has regular trade in it
	 *
	 */
	bool IsRegularHour() const;

	/**
	 *	@brief	Check if the bar only has vwap info but no high, low, open, close data (in case of speical trade only)
	 */
	bool IsVWAPOnly() const;
	 ...
	 */
	ACTIV_FEED_API_COMMON_API FieldSet::UpdateFieldSetStatus Update(const unsigned period, const Tick &tick, bool updateHighLow, bool updateVwap);
	 ...
	 *	@retval	UPDATE_STATUS_IGNORE
	 */
	ACTIV_FEED_API_COMMON_API FieldSet::UpdateFieldSetStatus Update(const unsigned period, const IntradayBar &bar);
	 ...
	 *	@retval	STATUS_CODE_INVALID_PARAMETER
	 */
	ACTIV_FEED_API_COMMON_API StatusCode RemoveTrade(const Tick &canceledTrade);
...
private:
	 ...
	 *	@brief	Set the bit in flag
	 */
	void SetNoneRegularTradeBit();

	/**
	 *	@brief	Remove the bit in flag
	 */
	void RemoveVWAPOnlyBit();

	/**
	 *	@brief	Remove the bit in flag
	 */
	void RemoveNoneRegularTradeBit();

	/**
	 *	@brief	check to see if a new bar should be created
	 */
	bool BelongToNextBar(const DateTime newDateTime, const unsigned period) const;
...
{
	return 0 == (m_flag & NONE_REGULAR_HOUR_TRADE_BIT);
}

// ---------------------------------------------------------------------------------------------------------------------------------

inline bool IntradayBar::IsVWAPOnly() const
{
	return 0 != (m_flag & VWAP_ONLY_BIT);
}
...
#endif // (!defined ACTIV_FEED_API_COMMON_INTRADAY_BAR_H)
