...
public:
	...
	public:
		 ...
		 */
		ReadHelper(const HeapMessage *pHeapMessage = 0, const size_t initialGetBufferSize = 0);
		 ...
		 *	@throw	StatusCodeException
		 */
		ReadHelper(const ReadHelper &rhs);
		 ...
		 *	@param	rhs the value to assign to this object.
		 */
		ReadHelper(const WriteHelper &rhs);
		 ...
		 *	@throw	StatusCodeException
		 */
		ReadHelper &operator=(const ReadHelper &rhs);
		 ...
		 *	@param	pHeapMessage the underlying heap message object.
		 */
		void Initialize(const HeapMessage *pHeapMessage);
	...
	public:
		 ...
		 *	@param	pHeapMessage the underlying heap message object.
		 */
		WriteHelper(HeapMessage *pHeapMessage = 0);
		 ...
		 *	@param	rhs the value to assign to this object.
		 */
		WriteHelper(const ReadHelper &rhs);
		 ...
		 *	@param	pHeapMessage the underlying heap message object.
		 */
		void Initialize(HeapMessage *pHeapMessage);
	 ...
	 *	@return	a reference to this object.
	 */
	HeapMessage &operator=(Proxy proxy);
	 ...
	 *	@return	a reference to this object.
	 */
	HeapMessage &operator=(HeapMessage &rhs);
	 ...
	 *	@return	whether the two objects are equal.
	 */
	bool operator==(const HeapMessage &rhs) const;
	...
	/**
	 *	@brief	Return a message to its heap if one is attached.
	 */
	void Return();
	 ...
	 *
	 *	@param	heapMessage reference to the message to be attached.
	 *
	 *	@retval STATUS_CODE_SUCCESS
	 *	@retval STATUS_CODE_INVALID_PARAMETER
	 */
	ACTIV_SYSTEM_API StatusCode Attach(HeapMessage &heapMessage);
...
private:
	...
	/**
	 *	@brief	Return the attached message to its heap.
	 */
	ACTIV_SYSTEM_API void InternalReturn();
...
inline void HeapMessage::Return()
{
	if (0 != m_pMessage)
		InternalReturn();
}
	...
	m_getBufferSize(0)
{
	...
	if (0 != initialGetBufferSize)
	{
		...
		if (STATUS_CODE_SUCCESS != statusCode)
			ACTIV_THROW(StatusCodeException, statusCode);
	}
}
