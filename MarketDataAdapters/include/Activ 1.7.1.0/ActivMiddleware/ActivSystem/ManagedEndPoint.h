 ...
 *
 *	A managed endpoint contains the underlying endpoint object and manages ports that are attached to the endpoint
...
public:
	...
	public:
		...
		uint32_t					m_threadPriority;				///< default: THREAD_PRIORITY_NORMAL
		Thread::ProcessorList		m_threadProcessorAffinityList;	///< default: all processors
#endif
	};
	...
	public:
		 ...
		 *
		 *	@throw	StatusCodeException if too many threads are processing the endpoint already, out of memory.
		 */
		 ...
		 *	@param	timeout how long to wait for messages on the endpoint, in ms, or ACTIV_WAIT_INFINITE.
		 *
		 *	@retval	STATUS_CODE_SUCCESS
