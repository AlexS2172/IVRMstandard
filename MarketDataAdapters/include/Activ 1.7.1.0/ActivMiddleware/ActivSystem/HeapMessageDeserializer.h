 ...
 *
 *	@param	MESSAGE_TYPE type of message to deserialize. Must have a StatusCode Deserialize(MessageValidator &) method.
...
private:
	...
	const void					*m_pData;
	size_t						m_dataLength;
	MessageValidator			m_messageValidator;
};
...
inline StatusCode HeapMessageDeserializer<MESSAGE_TYPE, BUFFER_SIZE>::Initialize(const HeapMessage &heapMessage, const MessageHandler::MessageVersion messageVersion)
{
	...
	m_messageValidator.Initialize(m_pData, m_dataLength, messageVersion);
...
{
	return message.Deserialize(m_messageValidator);
}
...
{
	return m_messageValidator.IsEndOfMessage();
}
