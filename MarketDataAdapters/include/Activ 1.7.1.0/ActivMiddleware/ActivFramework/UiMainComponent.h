...
protected:
	 ...
	 *	@brief	Main menu.
	 */
	class Menu : public UiMenu
	{
	public:
		/**
		 *	@brief	Constructor.
		 */
		ACTIV_FRAMEWORK_API Menu(UiMainComponent &uiComponent, const std::string &name, UiUser &uiUser);

	private:
		ACTIV_FRAMEWORK_API virtual void DisplayFooter(const UiIo &uiIo) const;
		ACTIV_FRAMEWORK_API virtual StatusCode GetOption(const UiIo &uiIo);

		UiMainComponent	&m_uiComponent;
	};
...
private:
	...
	typedef std::map<std::string, UiComponentInfo> UiComponentMap;
