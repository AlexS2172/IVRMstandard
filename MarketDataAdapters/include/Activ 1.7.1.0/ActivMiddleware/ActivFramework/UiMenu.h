...
protected:
	 ...
	 *
	 *	Checks for quit (if the menu was constructed with a quit option),
	 ...
	 *	@retval	...
	 */
	ACTIV_FRAMEWORK_API StatusCode ProcessOption(const UiIo &uiIo, const std::string &optionString);
...
private:
	 ...
	 *
	 *	The default behaviour is to get a string from the user and call ProcessOption() with it.
	 *	Override to add special option processing.
	 *
	 *	@param	uiIo to read / write to / from the user.
	 *
	 *	@retval	STATUS_CODE_SUCCESS option has been run.
	 ...
	 */
	virtual StatusCode GetOption(const UiIo &uiIo);
	 ...
	 */
	bool IsUserPermissionedForOption(const UiPermissions &optionUiPermissions) const;
