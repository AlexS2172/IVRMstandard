...
public:
	...
	public:
		...
		public:
			 ...
			 *	Zeros all members.
			 */
			ACTIV_FRAMEWORK_API SocketStats();

			static const MessageType MESSAGE_TYPE = ROUTER_MESSAGE_USER_NOTIFY_SOCKET_STATS;

			size_t		m_txQueueBytes;							///< current size of to transmit queue in bytes
			size_t		m_txQueueMessages;						///< current size of to transmit queue in number of messages
			size_t		m_txQueueHighBytes;						///< high size of to transmit queue in bytes
			size_t		m_txQueueHighMessages;					///< high size of to transmit queue in number of messages

			size_t		m_inTxQueueBytes;						///< current size of currently transmitting queue in bytes
			size_t		m_inTxQueueMessages;					///< current size of currently transmitting queue in number of messages
			size_t		m_inTxQueueHighBytes;					///< high size of currently transmitting queue in bytes
			size_t		m_inTxQueueHighMessages;				///< high size of currently transmitting queue in number of messages

			uint64_t	m_rxBytes;								///< total rx bytes
			uint64_t	m_txBytes;								///< total tx bytes

			size_t		m_mutexContentionCount;					///< approx. contentions on the socket's mutex
		};
		...
		/**
		 *	@brief	Enable receipt of socket stats for a circuit.
		 *
		 *	The component will receive a message of type RouterServiceApi::Tcp::SocketStats::MESSAGE_TYPE on a lazy basis.
		 *	Use DeserializeSocketStats() to retrieve the socket stats for the connection.
		 *
		 *	@param	component component to send request from.
		 *	@param	circuitAddress the address of the circuit to get the underlying socket stats for.
		 *
		 *	@return	STATUS_CODE_SUCCESS or an error status code.
		 */
		ACTIV_FRAMEWORK_API static StatusCode EnableSocketStats(const Component &component, const Address &circuitAddress);
		
		/**
		 *	@brief	Deserialize a socket stats message.
		 *
		 *	@param	heapMessage received message of type RouterServiceApi::Tcp::SocketStats::MESSAGE_TYPE
		 *	@param	socketStats reference to a SocketStats to receive the stats.
		 *
		 *	@return	STATUS_CODE_SUCCESS or an error status code.
		 */
		ACTIV_FRAMEWORK_API static StatusCode DeserializeSocketStats(const HeapMessage &heapMessage, SocketStats &socketStats);

		/**
		 *	@brief	Clear high counts for a socket.
		 *
		 *	@param	component component to send request from.
		 *	@param	circuitAddress the address of the circuit to get the underlying socket stats for.
		 */
		ACTIV_FRAMEWORK_API static StatusCode ClearSocketStatsHighCounts(const Component &component, const Address &circuitAddress);
