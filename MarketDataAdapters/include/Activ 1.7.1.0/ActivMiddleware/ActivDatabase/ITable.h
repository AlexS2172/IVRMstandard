...
public:
	 ...
	 *	@param	recordsPerRequest number of records to copy at a time
	 *
	 ...
	 *	@throw	OsException
	 */
	virtual ITable *Clone(const std::string &directory = std::string(), const bool createMutex = false, const bool copyRecords = true, size_t recordsPerRequest = 20) const = 0;
	 ...
	 */
	virtual StatusCode Copy(const ITable *pSourceTable, const bool clearTargetTable = true, size_t recordsPerRequest = 20) = 0;
	 ...
	 *
	 *	@param	messageValidator the message validator from which key data is extracted.
	 ...
	 *	@retval	STATUS_CODE_NOT_INITIALIZED
	 */
	virtual StatusCode Deserialize(MessageValidator &messageValidator, KeyList &keyList) const = 0;

	/**
	 *	@brief	Deserialize a list of records from a message.
	 *
	 *	@param	messageValidator the message validator from which the key and record data is extracted.
	 *	@param	recordList a list of records returned from table lookups.
	 *
	 *	@retval	STATUS_CODE_SUCCESS
	 *	@retval	STATUS_CODE_INVALID_KEY
	 ...
	 *	@retval	STATUS_CODE_NOT_INITIALIZED
	 */
	virtual StatusCode Deserialize(MessageValidator &messageValidator, RecordList &recordList) const = 0;

	/**
	 *	@brief	Deserialize record id and maximum response records used to return multiple records.
	 *
	 *	@param	messageValidator the message validator from which key and partial search data is extracted.
	 ...
	 *	@retval	STATUS_CODE_NOT_INITIALIZED
	 */
	virtual StatusCode Deserialize(MessageValidator &messageValidator, DataId &recordId, size_t &maxResponseRecords) const = 0;
	 ...
	 *	@retval	STATUS_CODE_NOT_INITIALIZED
	 */
	virtual StatusCode Deserialize(MessageValidator &messageValidator, std::vector<StatusCode> &statusCodeList, RecordList &recordList) const = 0;
	...
	/**
	 *	@brief	Check the integrity of the table.
	 *
	 *	@retval	STATUS_CODE_SUCCESS
	 *	@retval	STATUS_CODE_NOT_FOUND
	 */
	 ...
	 */
	void SetIdentity(const Identity identity = 0);
	 ...
	 *	@brief	Set mode.
	 *
	 *	@param	mode the new mode of the table.
	 */
...
protected:
	...
	Mode			m_mode;			///< The mode of the table.
...
inline void ITable::SetMode(const Mode mode)
{
	m_mode = mode;
}
...
inline Mode ITable::GetMode() const
{
	return m_mode;
}
