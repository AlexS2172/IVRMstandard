...
private:
	...
	public:
		 ...
		 *	@brief	Constructor.
		 *
		 *	@param	ioCompletionManager reference to parent IoCompletionManager.
		 */
		IoThread(IoCompletionManager &ioCompletionManager);

	protected:
		virtual StatusCode Process(void *pData);

	private:
		IoCompletionManager &	m_ioCompletionManager;			///< reference to parent IoCompletionManager
	};
	...
	typedef boost::shared_ptr<IoThread> IoThreadPtr;
	typedef std::vector<IoThreadPtr> IoThreadList;

	DWORD			m_concurrency;								///< number of processors in the system
	IoThreadList	m_ioThreadList;								///< list of io threads
