 ...
 */
class BinaryString : public IFieldType
{
public:
	 ...
	 *	@retval	STATUS_CODE_SUCCESS
	 */
	ACTIV_FIELD_TYPES_API StatusCode Compare(const BinaryString &rhs, int &result) const;
...
inline BinaryString::BinaryString() :
...
inline BinaryString::BinaryString(const uint8_t * const pData, const size_t size) :
	IFieldType(FIELD_TYPE),
	m_allocatedSize(0),
	m_pData(m_reservedBuffer),
	m_size(0)
...
inline BinaryString::BinaryString(const BinaryString &rhs) :
	IFieldType(FIELD_TYPE),
	m_allocatedSize(0),
	m_pData(m_reservedBuffer),
	m_size(0)
...
inline BinaryString& BinaryString::operator=(const BinaryString &rhs)
{
	if (this != &rhs)
	{
		if (rhs.IsInitialized())
...
inline StatusCode BinaryString::Set(const uint8_t * const pData, const size_t size)
{
	if (0 == size)
...
inline StatusCode BinaryString::Get(const uint8_t *&pData, size_t &size) const
{
	if (!IsInitialized())
...
inline const uint8_t *BinaryString::GetData() const
{
	if (!IsInitialized())
...
inline size_t BinaryString::GetSize() const
{
	if (!IsInitialized())
		ACTIV_THROW(StatusCodeException, STATUS_CODE_NOT_INITIALIZED);
...
inline void BinaryString::Reset()
{
	m_size = 0;
	m_isInitialized = false;
}
...
inline void BinaryString::Clear()
{
	if (0 != m_allocatedSize)
		std::free(m_pData);
...
inline size_t BinaryString::GetSerializedLength() const
{
...
inline size_t BinaryString::GetSerializedBodyLength() const
{
...
inline size_t BinaryString::GetMaxSerializedLength(const size_t size)
{
...
inline size_t BinaryString::GetMaxSerializedBodyLength(const size_t size)
{
	return size;
}
...
inline bool BinaryString::IsValidSerializedBodyLength(const size_t serializedBodyLength)
{
