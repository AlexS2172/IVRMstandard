 ...
 *	@file	BinaryArray.h
 *
 *	@brief	Header file for the binary array class.
 *
 *	$Log: $
 */

#if !defined (ACTIV_BINARY_ARRAY_H)
#define ACTIV_BINARY_ARRAY_H
...
/**
 *	@brief	Binary array class.
 */
class BinaryArray : public IFieldType
{
public:
	static const FieldType FIELD_TYPE = FIELD_TYPE_BINARY_ARRAY; ///< The field type.
	 ...
	 */
	BinaryArray(const uint8_t * const pData, const size_t size);
	 ...
	 */
	BinaryArray& operator=(const BinaryArray &rhs);
	 ...
	 *	@retval	STATUS_CODE_SUCCESS
	 */
	ACTIV_FIELD_TYPES_API StatusCode Compare(const BinaryArray &rhs, int &result) const;
...
inline BinaryArray::BinaryArray() :
...
inline BinaryArray::BinaryArray(const uint8_t * const pData, const size_t size) :
	IFieldType(FIELD_TYPE),
	m_allocatedSize(0),
	m_pData(m_reservedBuffer),
	m_size(0)
...
inline BinaryArray::BinaryArray(const BinaryArray &rhs) :
	IFieldType(FIELD_TYPE),
	m_allocatedSize(0),
	m_pData(m_reservedBuffer),
	m_size(0)
...
inline BinaryArray& BinaryArray::operator=(const BinaryArray &rhs)
{
	if (this != &rhs)
	{
		if (rhs.IsInitialized())
...
inline StatusCode BinaryArray::Set(const uint8_t * const pData, const size_t size)
{
	if (0 == size)
...
inline StatusCode BinaryArray::Get(const uint8_t *&pData, size_t &size) const
{
	if (!IsInitialized())
...
inline const uint8_t *BinaryArray::GetData() const
{
	if (!IsInitialized())
...
inline size_t BinaryArray::GetSize() const
{
	if (!IsInitialized())
		ACTIV_THROW(StatusCodeException, STATUS_CODE_NOT_INITIALIZED);
...
inline size_t BinaryArray::GetSerializedLength() const
{
...
inline size_t BinaryArray::GetSerializedBodyLength() const
{
...
inline size_t BinaryArray::GetMaxSerializedLength(const size_t size)
{
...
inline size_t BinaryArray::GetMaxSerializedBodyLength(const size_t size)
{
	return size;
}
...
inline bool BinaryArray::IsValidSerializedBodyLength(const size_t serializedBodyLength)
{
...
#endif // !defined (ACTIV_BINARY_ARRAY_H)
