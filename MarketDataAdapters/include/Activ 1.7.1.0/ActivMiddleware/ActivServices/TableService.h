...
#if (!defined ACTIV_SERVICES_TABLE_SERVICE_H)
#define ACTIV_SERVICES_TABLE_SERVICE_H
...
inline TableService::ThreadInfo *TableService::GetThreadInfo() const
{
	ThreadInfo *pThreadInfo = m_threadInfo.Get();

	if (0 == pThreadInfo)
		pThreadInfo = AllocateThreadInfo();
...
#endif // (!defined ACTIV_SERVICES_TABLE_SERVICE_H)
