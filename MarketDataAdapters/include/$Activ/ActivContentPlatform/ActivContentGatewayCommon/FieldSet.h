...
{
public:
	const static size_t											SIZEOFBYTE = sizeof(byte_t);
	ACTIV_CONTENT_GATEWAY_COMMON_API const static std::string	DEFAULT_STRING_FIELD_SEPARATER;
	 ...
	 *	@brief	Constructor.
	 */
	ACTIV_CONTENT_GATEWAY_COMMON_API FieldSet();
	 ...
	 */
	ACTIV_CONTENT_GATEWAY_COMMON_API void SetSequence(byte_t sequence);
	 ...
	 */
	ACTIV_CONTENT_GATEWAY_COMMON_API byte_t GetSequence() const;
...
public:
	 ...
	 *	@retval	...
	 */
	ACTIV_CONTENT_GATEWAY_COMMON_API static StatusCode ReadInteger(const std::string &input,  const std::string &deliminator, std::string::size_type &startPos, uint32_t &uintOut);
	...
	ACTIV_CONTENT_GATEWAY_COMMON_API static StatusCode FindContentSubstring(const std::string &input,  const std::string &deliminator, std::string::size_type &startPos, std::string &subStringOut);
...
inline int64_t abs64(int64_t value)
{
...
inline StatusCode FieldSet::ReadDate(const std::string &input,  const std::string &deliminator, std::string::size_type &startPos, Date &dateOut)
{
	std::string subString;
	ACTIV_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, FieldSet::FindContentSubstring(input, deliminator, startPos, subString));
	uint32_t year, month, day;
	char temp;
	int read = sscanf(subString.c_str(), "%04d%c%02d%c%02d", &year, &temp, &month, &temp, &day);
	if(read < 5)
		return STATUS_CODE_INVALID_PARAMETER;
	return dateOut.Set(year, month, day);
}

// ---------------------------------------------------------------------------------------------------------------------------------

inline StatusCode FieldSet::ReadDateTime(const std::string &input,  const std::string &deliminator, std::string::size_type &startPos, DateTime &dateTimeOut)
{
	std::string subString;
	ACTIV_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, FieldSet::FindContentSubstring(input, deliminator, startPos, subString));
	uint32_t year, month, day, hour, minute, second;
	char temp;
	int read = sscanf(subString.c_str(), "%04d%c%02d%c%02d%c%02d%c%02d%c%02d", &year, &temp, &month, &temp, &day, &temp, &hour, &temp, &minute, &temp, &second);
	if(read < 11)
		return STATUS_CODE_INVALID_PARAMETER;
	return dateTimeOut.Set(year, month, day, hour, minute, second);
}
