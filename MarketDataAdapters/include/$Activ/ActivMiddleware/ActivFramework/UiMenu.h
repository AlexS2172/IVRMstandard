...
public:
	...
	public:
		 ...
		 *	@brief	Constructor.
		 */
		ACTIV_FRAMEWORK_API OptionAttributes(const UiPermissions &uiPermissions = UiPermissions::READ);
	 ...
	 */
	template<typename T>
	void AddOption(const std::string &name, T &object, void (T::*pFunction)(const UiIo &),
				   const OptionAttributes &optionAttributes = OptionAttributes())
	{
		// NB VC++ 7.0 limitation, have to have this inline here
		if (!IsUserPermissionedForOption(optionAttributes.m_uiPermissions))
			return;
	 ...
	 */
	ACTIV_FRAMEWORK_API void AddOption(const std::string &name, GlobalOptionFunction pFunction,
									   const OptionAttributes &optionAttributes = OptionAttributes());

	/**
	 *	@brief	Run the menu.
	 *
	 *	@param	pParentMenu menu launching this menu.
	 */
...
protected:
	 ...
	 */
	ACTIV_FRAMEWORK_API virtual void DisplayOption(const UiIo &uiIo, const size_t optionIndex, const std::string &name) const;
