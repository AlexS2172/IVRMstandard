...
#if !defined (ACTIV_PAGING_FILE_MAPPED_MEMORY_FACTORY_H)
#define ACTIV_PAGING_FILE_MAPPED_MEMORY_FACTORY_H
 ...
 */
class ACTIV_BASE_API PagingFileMappedMemoryFactory
{
public:
	/**
	 ...
	 *	@throw	Exception
	 */
	IMappedMemory *Create(const std::string &name, const uint64_t size, const IMappedMemory::MappingFlags mappingFlags = IMappedMemory::MAPPING_FLAG_NONE);
	 ...
	 *	@throw	Exception
	 */
	IMappedMemory *Create(const uint64_t size, const IMappedMemory::MappingFlags mappingFlags = IMappedMemory::MAPPING_FLAG_NONE);
	 ...
	 *	@throw	BaseException
	 */
	IMappedMemory *Open(const std::string &name, const IMappedMemory::MappingFlags mappingFlags = IMappedMemory::MAPPING_FLAG_NONE, Timeout &timeout = Timeout::GetInfinite());

	/**
	 *	@brief	Get the instance of the factory object.
	 */
	static PagingFileMappedMemoryFactory &GetInstance();
...
#endif // !defined (ACTIV_PAGING_FILE_MAPPED_MEMORY_FACTORY_H)
