 ...
 */
class ACTIV_BASE_API FastSharedMutexFactory
{
public:
	/**
	 ...
	 *	@throw	Exception.
     */
	IMutex *Create(const std::string &name, const bool isInitialOwner = false);
	 ...
	 *	@throw	Exception.
     */
	IMutex *Open(const std::string &name, Timeout &timeout = Timeout::GetInfinite());

	/**
	 *	@brief	Get the instance of the factory object.
	 */
	static FastSharedMutexFactory &GetInstance();
