// MyTrackProviders.cpp : Implementation of WinMain
 

// Note: Proxy/Stub Information
//      To build a separate proxy/stub DLL, 
//      run nmake -f MyTrackProvidersps.mk in the project directory.
#include "stdafx.h"
#include "resource.h"
#include <initguid.h>
#include "MyTrackProviders.h"

#include "MyTrackProviders_i.c"
...
BEGIN_OBJECT_MAP(ObjectMap)
	OBJECT_ENTRY(CLSID_MyTrackPriceProvider, CMyTrackPriceProvider)
	OBJECT_ENTRY(CLSID_MyTrackStructureProvider, CMyTrackStructureProvider)
	OBJECT_ENTRY(CLSID_MyTrackStructureInfo, CMyTrackStructureInfo)
	OBJECT_ENTRY(CLSID_MyTrackPriceInfo, CMyTrackPriceInfo)
	OBJECT_ENTRY(CLSID_ProviderInfo, CProviderInfo)
	OBJECT_ENTRY(CLSID_MyTrackPriceInfoWithNotify, CMyTrackPriceInfoWithNotify)
END_OBJECT_MAP()
    ...
    HINSTANCE /*hPrevInstance*/, LPTSTR lpCmdLine, int /*nShowCmd*/)
{
	...
	g_hCheckMutex = ::CreateMutex(NULL, FALSE, _T("MyTrackPtoviderMutex"));
	//if(GetLastError()!=ERROR_ALREADY_EXISTS )
	{

		_ASSERTE(SUCCEEDED(hRes));
		_Module.Init(ObjectMap, hInstance, &LIBID_MYTRACKPROVIDERSLib);
		...
		while (lpszToken != NULL)
		{
			...
			{
				_Module.UpdateRegistryFromResource(IDR_MyTrackProviders, FALSE);
			...
			{
				_Module.UpdateRegistryFromResource(IDR_MyTrackProviders, TRUE);
