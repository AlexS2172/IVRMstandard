...
DWORD WINAPI CEgTcpDriver::Start(IEgTcpDriverEvents * pEvents, 
								 ...
								 const u_long ulServerAddr)
{
	DWORD dwExtendedErrorCode = 0L;
	return Start(pEvents, ThreadingModel, dwExtendedErrorCode, usServerPort, ulServerAddr);
}
...
DWORD WINAPI CEgTcpDriver::Start(IEgTcpDriverEvents * pEvents, 
								 ...
								 const u_long ulServerAddr)
{
	dwExtendedErrorCode = 1;
	if (pEvents == NULL)
		return ERROR_INVALID_PARAMETER;

	CAutoLock lock(m_InitLock);

	dwExtendedErrorCode = 2;
	if (m_bInited)
	...
	m_ulServerAddr = ulServerAddr;

	dwExtendedErrorCode = 3;
	...
	{
		dwExtendedErrorCode = 7;
		DWORD dwErr = CreateListenSocket();
		if (dwErr != 0)
			return dwErr;

		dwExtendedErrorCode = 8;
...
DWORD WINAPI CEgTcpDriver::Stop()
{
	...
	for(long i = 0; i < WSATOTAL_EVENTS; i++)
...
DWORD WINAPI CEgTcpDriver::ResolveAddress2(const char * const pServer, 
										  const unsigned short usPort,
...
DWORD WINAPI CEgTcpDriver::ResolveName(long lAddress, char* const pName, int iLen)
{
	...
	strcpy_s(pName,iLen, pAddr->h_name);
...
DWORD WINAPI CEgTcpDriver::Send(SOCKET Sock, const LPVOID pData, const DWORD dwDataLen, BOOL bSync)
{
	if (!m_bInited)
		return 0;//0 is needed for Transport
			...
			default:
				...
				switch(dwEvent)
				{
					...
					case WSAEVENTSELECT_EVENT:
						...
						if(pThis->m_SocketContexts.empty())	
							WSAResetEvent(pThis->m_hWsaEvents[WSAEVENTSELECT_EVENT]);
												  ...
												  it2 != ClosedSockets.end(); it2++)
						{
							...
							set_contexts_t::iterator it = pThis->m_SocketContexts.find(FindSocketContext);
						...
						for(set<SOCKET>::iterator it2 = ConnectedSockets.begin();
...
DWORD WINAPI CEgTcpDriver::CreateListenSocket()
{
    ...
    si_addrlocal.sin_addr.s_addr = m_ulServerAddr;
